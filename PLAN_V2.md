...
| 5 | **편집 신호** | 사람이 매번 지우는 표현 | 보류 | 별도 승인 후 재설계 |
...
기존 테이블(`users` `studio_state` `products` `product_proofs` `copy_selections`)은 **건드리지 않는다.** 위에 얹는다.
...
## 8. 결정된 것 (2026-08-24, 요청자)

### ① 배포한다 — Vercel ⚠️ **2026-08-10 결정을 뒤집는다**

`CLAUDE.md` 머리말의 "⚠️ 배포하지 않는다. 관계자가 각자 로컬에서 띄워 쓴다" 는 **폐기다.**
여러 사람이 쓰는 게 목적이 된 이상 로컬 실행은 그 자체가 병목이다. `vercel.json` 은 이미 있다.

배포가 데려오는 것 넷 — 구현 전에 정리해 둔다.

- **개인 키 방식(8-7)의 전제가 바뀐다.** 외부 AI 왕복이 기본 경로라 앱은 키 없이도 끝까지 돈다.
  앱 내 AI(③)를 쓰는 사람만 자기 키를 넣는다. `config.local.js` 내장 키는 **배포본에 올리지 않는다.**
- **이제 RLS 가 유일한 보호막이다.** 로컬 실행일 때는 URL 을 아는 사람이 없다는 게 사실상의 울타리였다.
  공개 URL 이 생기면 `users.status='approved'` 승인제 + RLS 만 남는다. 각 Phase 검증에 계정 2개
  교차 확인을 고정하는 이유가 여기 있다.
- **Supabase 프로젝트를 하나로 모아야 한다.** 지금은 각자 자기 프로젝트를 만들어 쓰는 안내
  (`SUPABASE_SETUP.md`)다. 팀 공용 메모리는 **한 프로젝트**를 봐야 성립한다. 안내 문서를
  "각자 설치"에서 "초대받아 로그인"으로 다시 쓴다.
- **`/api/research/*`(스타일 수집)가 살아난다.** 이 PC 에 Node 가 없어 못 돌리던 STEP 이
  Vercel 에서는 그냥 된다. 로컬 개발에서만 여전히 Node 가 필요하다.

### ② 왕복 1회 — 브리프 하나로 4종을 다 받는다

2-2 계약 그대로. 폴백(3-1 의 3단, 3-3 의 보정)은 계약대로 안 온 원고를 위해 그대로 둔다.

### 남은 결정

3. **공유 기본값.** 저장 시 팀 공개가 기본(권장) vs 개인 비공개가 기본. — Phase 2 시작 전까지
4. **승인자.** admin 은 요청자 1인인가. — Phase 3 시작 전까지
5. **앱 내 AI 경로.** 유지(권장 · 폴백) vs 제거(코드 단순화). — 급하지 않다
