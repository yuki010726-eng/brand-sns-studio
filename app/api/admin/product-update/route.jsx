/**
 * 상품 관리 탭의 서버 쪽 절반 — 옛 api/admin/product-update.mjs (Vercel `(req,res)` 핸들러)를
 * Next.js App Router Route Handler 로 옮겼다. 클라이언트가 고른 문장을 그대로 믿지 않고
 * 링크 원문을 서버에서 다시 읽어 대조한 뒤에만 Supabase 에 저장한다.
 *
 * ⚠️ 다른 마이그레이션된 라우트(`app/api/research/collect/route.jsx`, `app/api/text/route.jsx`)와
 *    같은 패턴이다 — `api/_shared.mjs` 의 Vercel 스타일 헬퍼(`requirePost`/`fail`/`json`/`requireAdminUser`)를
 *    재사용하지 않고, 이 파일 안에서 Fetch API(`Request`/`Response`) 스타일로 다시 구현한다.
 *    다만 네이버 블로그 파싱(`fetchText`/`parseArticle`/`canonicalBlogUrl`/`mobileBlogUrl`)은
 *    research 라우트와 같은 `api/research/_shared.mjs` 를 그대로 재사용한다 — 두 엔드포인트가
 *    같은 로직을 따로 들고 있으면 한쪽만 고치는 실수가 난다.
 * ⚠️ 여기는 **관리자 전용**이다 — `requireApprovedUser` 만으로는 부족하고 `role === 'admin'` 까지 본다.
 */
import { SUPABASE } from "../../../../lib/supabase.js";
import { canonicalBlogUrl, fetchText, mobileBlogUrl, parseArticle } from "../../../../api/research/_shared.mjs";
...
async function requireAdminUser(request) {
  ...
  try {
    const profileResponse = await fetch(
      `${supabaseUrl}/rest/v1/users?id=eq.${encodeURIComponent(user.id)}&select=status,role`,
      { headers },
    );
  ...
  return { ok: true, headers, supabaseUrl };
}
...
export async function POST(request) {
  ...
  const urls = [
    ...new Set((Array.isArray(body?.urls) ? body.urls : []).map(canonicalNaverUrl).filter(Boolean)),
  ].slice(0, 10);
  const approvedSources = approvedSourceMap(body?.sources);
  if (!productId) return fail(400, "상품을 선택해 주세요.");
  if (!urls.length) return fail(400, "올바른 네이버 블로그 또는 카페 링크를 입력해 주세요.");

  const existing = await existingProductData(auth, productId);
  if (!existing) return fail(404, "상품을 찾을 수 없습니다.");

  const items = await collectItems(urls, existing.proofs);
  const succeeded = items.filter((item) => !item.error);
  if (!succeeded.length) return Response.json({ items, error: "확인할 수 있는 링크가 없습니다." }, { status: 422 });
  const newCount = succeeded.reduce((sum, item) => sum + item.newContent.length, 0);

  if (action === "preview") return Response.json({ action, items, newCount });
  ...
  return Response.json({
    action,
    items: [...changed, ...items.filter((item) => item.error)],
...
async function existingProductData(auth, productId) {
  const response = await fetch(
    `${auth.supabaseUrl}/rest/v1/products?id=eq.${encodeURIComponent(productId)}&select=id,product_proofs(content),product_sources(source_url,source_content)`,
  ...
  const sources = new Map(
    (Array.isArray(product.product_sources) ? product.product_sources : []).map((source) => [
      source.source_url,
      Array.isArray(source.source_content) ? source.source_content : [],
    ]),
  );
...
async function collectItems(urls, existingProofs) {
  ...
  for (const url of urls) {
    try {
      ...
      items.push({ url, error: error.message || "가져오지 못했습니다." });
    }
  }
  return items;
}
...
async function resolveCafeId(alias) {
  if (!alias) return "";
  const response = await fetch(`https://apis.naver.com/cafe-web/cafe2/CafeGateInfo.json?cluburl=${encodeURIComponent(alias)}`, {
