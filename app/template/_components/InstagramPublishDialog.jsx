...
export function InstagramPublishDialog({ open, images, caption, busy, onCaptionChange, onClose, onPublish }) {
  if (!open) return null;
  return (
      ...
      <div className="max-h-[90dvh] w-full max-w-[720px] overflow-auto rounded-[18px] bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[#6b7280]">아래 순서대로 피드 캐러셀에 바로 게시됩니다.</p>
          </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="button" onClick={onPublish} disabled={busy} className="rounded-full bg-[#287aff] px-5 py-2.5 text-[14px] font-bold text-white disabled:opacity-40">{busy ? '게시 중…' : 'Instagram에 게시'}</button>
        </div>
      </div>
    </div>
  );
