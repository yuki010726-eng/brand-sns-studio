        ...
        {steps.map((step, index) => (
          <li key={step.path} className="max-[860px]:contents">
            ...
            <div
              ...
              className={`flex items-center gap-3 rounded-full px-2 py-2 text-sm ${index === activeIndex ? "bg-[#1a1a1a] font-bold text-white" : "text-white/45"}`}
            >
              <span
                className={`grid size-[22px] place-items-center rounded-full text-xs font-bold ${index === activeIndex ? "bg-white text-[#1a1a1a]" : "bg-white/30 text-white"}`}
              >
                {step.n}
              </span>
