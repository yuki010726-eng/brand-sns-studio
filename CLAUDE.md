...
| 4 | `pages/template.js` + `lib/cardrender.js` | ✅ 완료 |
| 5 | `pages/library.js` | ⬜ **자리표시자 — 다음 작업** (보관함 검색·필터·정렬) |
...
**템플릿 3종 선택 → 카드 탭(01~06) → 2열 편집.** 왼쪽 미리보기 canvas, 오른쪽 슬롯별 입력칸.
...
| `card` 카드형 | soosangmarket | **표지** 파랑 그라데이션 + 테두리 pill + 흰 제목 / **본문** 파란 박스(대주제) + 흰 박스(검은 소주제) / **마무리** 파랑 단색 + 중앙 팔로우 문구 (이미지 안 씀) |
| `note` 노트형 | sslmo.lab | **표지** 종이 배경 + 중앙 제목·부제 + 큰 아이콘 이미지 / **본문** 좌상단 아이콘 + 아주 굵은 검은 제목 + 연한 회색 본문 + **검정 하이라이트 바** |
