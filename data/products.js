/**
 * 4개 상품 기준 정보
 * 출처: 공식 제안서·브로셔 8종 + openxgroup.co.kr 브랜드 페이지 (2026-08-13 전면 검증)
 *   - 2026_KBS N 브랜드어워즈_제안서 / KBS N 브랜드 팩트시트(2026-07-20)
 *   - 포브스 5종 브로셔 (혁신대상 / 최고의 브랜드 / 프리미엄 브랜드 / 사회공헌대상 / CEO 리더십 경영대상)
 *   - 2026 대한민국 고객만족 신뢰도 대상_제안서
 *   - 2026 중소기업프로젝트_TV CF_제안서
 * 통합 팩트시트: 바탕화면/card_news_gpt_package/04_KNOWLEDGE_브랜드_정보_v2_통합.md
 * 게시 시점에 일정·접수 상태는 공식 자료로 다시 확인해야 한다.
 *
 * ⚠️ 비용 규정 개정 (요청자 결정 2026-08-14) — **막는 것은 '금액'이지 '비용이라는 주제'가 아니다.**
 *    2026-08-13 판의 전면 금지는 폐기했다.
 *    ⭕ "비용은 상담 시 안내드립니다" / "부담 없이 상담 남겨보세요" — 다음 행동을 안내하는 문장은 쓴다.
 *    ❌ 구체 금액 / "저렴한"·"가성비"·"부담 없는 **비용**" — 가격을 주장하는 순간 표시광고 리스크가 붙는다.
 *    ⚠️ '부담 없는 상담'(행동 안내)과 '부담 없는 비용'(가격 주장)은 다르다. 이 한 칸이 규정의 경계다.
 *    왜 열었나: 비용을 아예 안 꺼내면 "말 안 하는 게 더 수상하다"로 읽히고,
 *    콘텐츠의 목적지인 상담을 안내할 문장 자체가 사라진다.
 */

/** @typedef {'kbsn'|'forbes'|'kcst'|'aitvcf'} ProductId */

export const PRODUCTS = [
  {
    id: 'kbsn',
    name: 'KBS N 브랜드어워즈',
    short: 'KBS N',
    icon: 'award',
    handle: '@kbsnawards_official',
    tagline: '방송사 최초 브랜드어워즈',
    summary: 'KBS N과 주식회사 오픈엑스가 공동 진행하는 브랜드 시상식. 연중 상시 접수이며 2026년 시상식은 12월 18일 상암 스탠포드호텔에서 열립니다.',
    intake: '연중 상시 접수',
    site: 'https://kbsnawards.com/',
    facts: [
      '2026년 시상식 예정일 · 2026.12.18',
      '예정 장소 · 상암 스탠포드호텔',
      '수상기업 수 · 2023년 108개 / 2024년 139개',
    ],
    benefits: ['엠블럼 5종', '인증서', '시상식 참석', '수상 기사', 'KBS N 홈페이지 게재', '인터뷰 영상'],
    packages: [
      { name: '패키지 1', desc: 'KBS 드라마 자막광고 형태의 슈퍼자막 (약 2~3초 노출)' },
      { name: '패키지 2', desc: '15초 AI 홍보영상' },
    ],
    appeals: ['KBS 공신력', '방송사 최초', 'TV 노출', '브랜드 신뢰 강화', '제3자 인증'],
    cautions: [
      '기본 특전과 추가 패키지를 구분해서 표현',
      '수상만으로 매출이 상승한다고 단정하지 않음',
      '공식 근거 없이 최고·유일·1위 표현을 사용하지 않음',
    ],
    /**
     * 마무리 문장 — 공통 사실성 원칙의 '권장 표현'만 사용한다.
     *
     * ⚠️ 2 → 5개로 늘렸다 (2026-08-14, 요청자 승인). 2개뿐일 때는 글을 열 편 뽑으면
     *    다섯 편씩 똑같이 끝났다. 늘릴 때 지키는 선은 그대로다 —
     *    **성과·매출을 보장하지 않고, 비용을 꺼내지 않고, 자료에 있는 사실만 근거로 쓴다.**
     *    3~5번은 각각 특전 활용 / 접수 시점 / 판단 순서로 각도를 나눠 두었다.
     */
    closings: [
      '수상 이력은 고객이 브랜드를 판단할 때 신뢰 근거로 활용될 수 있습니다.',
      '브랜드의 품질과 경쟁력을 객관적으로 보여주는 자료가 될 수 있습니다.',
      '엠블럼과 인증서는 제품과 매장, 온라인 채널에서 이어서 활용할 수 있습니다.',
      '접수는 연중 상시라 준비가 되는 시점에 맞춰 검토하셔도 됩니다.',
      '지금 필요한 것이 인증인지 노출인지부터 정하면 선택이 한결 쉬워집니다.',
    ],
    /**
     * ⚠️ **첫 번째가 대표 태그다.** `tagsFor()` 가 이것만 고정으로 넣고 나머지를 돌린다.
     *    5 → 14개로 늘렸다 (2026-08-14, 요청자 승인). 5개일 때는 인스타가 5개를 다 쓰는 바람에
     *    **순서만 돌 뿐 늘 같은 태그**가 나갔고, 전부 브랜드 이름이라 이 브랜드를 이미 아는
     *    사람에게만 닿았다. 뒤쪽은 새로 들어올 사람이 검색하는 **탐색 태그**다.
     *    ⚠️ 태그도 사실성 원칙 안에 있다 — 비용·성과·최고/1위를 암시하는 태그를 넣지 말 것.
     */
    hashtags: [
      'KBSN브랜드어워즈', 'KBSN', '브랜드어워즈', '기업인증', '브랜드신뢰도',
      '브랜드마케팅', '기업브랜딩', '중소기업마케팅', '기업홍보', '브랜드전략',
      '브랜드가치', '시상식', '수상기업', '마케팅인사이트',
    ],
    /**
     * SNS용 화법 재료 — 불릿을 나열하는 대신 이 문장들을 조합해 글을 만든다.
     * hooks 는 톤 순서(신뢰/후킹/담백/축하)에 맞춰 4개를 둔다.
     */
    voice: {
      hooks: [
        '2024년 한 해에만 139개 기업이 이 상을 받았습니다.',
        '"수상 이력, 그래서 어디에 쓰나요?"',
        '올해 시상식은 12월 18일, 상암입니다.',
        '📢 2026 KBS N 브랜드어워즈, 접수 진행 중입니다.',
      ],
      /** 블로그 소제목용 — 질문과 답을 반드시 쌍으로 둔다 (따로 뽑으면 서로 어긋난다) */
      qa: [
        { q: 'KBS N 브랜드어워즈는 어떤 상인가요?',
          a: '방송사가 직접 여는 브랜드어워즈로, KBS N과 주식회사 오픈엑스가 함께 진행합니다. 2023년에는 108개, 2024년에는 139개 기업이 수상했습니다.',
          shot: 'a modern television broadcast control room, rows of glowing monitors, blue ambient light' },
        { q: '수상하면 무엇을 받게 되나요?',
          a: '엠블럼 5종과 인증서, 시상식 참석, 수상 기사, KBS N 홈페이지 게재, 인터뷰 영상이 기본 특전으로 제공됩니다.',
          shot: 'an award certificate and a polished metal emblem badge arranged on a dark wooden desk, soft window light' },
        { q: '추가 패키지는 뭐가 다른가요?',
          a: '기본 특전과는 별개입니다. KBS 드라마 자막광고 형태의 슈퍼자막을 약 2~3초 노출하는 패키지 1, 15초 AI 홍보영상을 만드는 패키지 2가 있습니다.',
          shot: 'a living room television playing a drama scene in a dim evening room, glowing screen' },
        { q: '시상식은 언제, 어디서 열리나요?',
          a: '2026년 시상식은 12월 18일 상암 스탠포드호텔에서 열릴 예정입니다. 접수는 연중 상시라 지금 준비를 시작해도 늦지 않습니다.',
          shot: 'an elegant hotel banquet hall prepared for an evening awards ceremony, round tables and stage lighting' },
        { q: '받은 다음에는 어디에 쓰나요?',
          a: '엠블럼과 인증서는 제품 패키지나 상세페이지, 명함, 매장 게시물에 들어갑니다. 수상 기사와 인터뷰 영상은 브랜드 소개 자료로 이어서 활용할 수 있습니다.',
          shot: 'a product package box and business cards laid out on a clean studio table, soft shadows' },
      ],
      /** 표지·반론·마무리 카드의 장면 (영문 — 이미지 생성 프롬프트에 그대로 들어간다) */
      shots: {
        cover: 'a grand awards ceremony stage in a broadcast hall, warm golden spotlights, silhouetted audience, empty podium',
        note: 'a small business owner sitting alone at a desk late at night, laptop glow on their face, thoughtful',
        outro: 'a shop entrance at dusk with a framed certification plaque mounted beside the door',
      },
      proof: [
        // ⚠️ 재료가 적으면 주제를 바꿔도 같은 글이 나온다 — forbes proof 주석 참고
        // 주최·성격
        '방송사가 직접 여는 브랜드어워즈로, KBS N과 주식회사 오픈엑스가 함께 진행합니다.',
        '대한민국 방송사 중 최초로 마련된 기업 대상 브랜드 어워즈입니다.',
        '인증 주체가 민간 협회나 개별 매체가 아니라 KBS 그룹의 KBS N이라는 점이 신뢰의 근거입니다.',
        // 규모
        '2023년에는 108개, 2024년에는 139개 기업이 수상했습니다.',
        '단발성 행사가 아니라 매년 규모가 커지고 있습니다.',
        // 심사·자격
        '각 분야별 상품·서비스 만족도를 기준으로 우수 기업을 선정합니다.',
        '심사가 소비자 평가에 근거한다는 점이 인증의 설득력이 됩니다.',
        '일반기업뿐 아니라 공기업, 지방자치단체, 교회, 병·의원도 참여할 수 있습니다.',
        '성인 업종과 도박 등 사행성을 조장하는 업종은 참여할 수 없습니다.',
        '참여신청서와 사업자등록증, 수상기사 요청서를 제출합니다.',
        '심사는 개별심사로 진행되고 결과는 개별 통보됩니다.',
        // 특전 (항목마다 쓰임새가 다르다)
        '심사를 통과하면 엠블럼 5종과 인증서, 수상 기사, KBS N 홈페이지 게재, 인터뷰 영상까지 함께 제공됩니다.',
        '엠블럼은 제품 상세페이지나 명함, 홈페이지에 붙여 쓸 수 있습니다.',
        '수상 기사는 검색 결과에 남는 제3자 기록이 됩니다.',
        'KBS N 공식 홈페이지에 수상 콘텐츠가 게재됩니다.',
        '어워즈 통합기사와 수상 기업 개별기사가 온라인 언론매체로 송출됩니다.',
        '인증서는 온라인용과 오프라인 상장·상패로 함께 나옵니다.',
        // 연계 상품
        '원하면 KBS 드라마 슈퍼자막이나 15초 AI 홍보영상을 추가 패키지로 붙일 수 있습니다.',
        '슈퍼자막은 KBS DRAMA와 KBS STORY의 드라마가 끝나는 시점에 브랜드 자막을 노출합니다.',
        'AI 홍보영상은 촬영 없이 기획과 AI 기술만으로 15초 CF를 만듭니다.',
        // 일정
        '접수는 연중 상시라 지금 준비를 시작해도 늦지 않습니다.',
        '2026년 시상식은 12월 18일 상암 스탠포드호텔에서 열립니다.',
        '시상식은 상암 스탠포드호텔 그랜드볼룸에서 진행됩니다.',
        // 슈퍼자막 상세
        '슈퍼자막은 주말드라마·미니시리즈 묶음과 일일드라마 묶음 중에서 고릅니다.',
        '자막 소재는 가로 1225, 세로 175 픽셀로 만듭니다.',
        '자막 소재는 온에어 7일 전까지 넘겨야 합니다.',
        '드라마 편성은 매월 달라질 수 있어 송출 시점은 스케줄에 따라 지정됩니다.',
        // AI 영상 상세
        'AI 영상은 콘티 기획, 컷별 이미지, 컷별 영상, 후보정을 거쳐 완성됩니다.',
        '브랜드 톤앤매너를 학습시켜 캐릭터와 분위기를 일관되게 유지합니다.',
        '재촬영 없이 프롬프트만 고쳐 날씨나 배경, 모델을 바꿀 수 있습니다.',
        '동물병원, 헬스장, 안과, 프랜차이즈 음식점 같은 업종에서 만든 사례가 있습니다.',
        '손소독제, 로봇청소기, 화장품처럼 제품을 앞세운 영상도 만듭니다.',
        // 자산·활용
        '엠블럼은 온·오프라인 홍보용으로 5종이 이미지 파일로 나옵니다.',
        '상장과 상패는 오프라인 신뢰 자산으로 매장이나 사무실에 둡니다.',
        'KBS N에는 Drama, Joy, Sports, Story, Kids, Life 여섯 개 채널이 있습니다.',
        '한 번 받고 끝나는 상이 아니라 여러 해 이어서 받는 기업도 있습니다.',
        '수상 이후에도 기사와 영상이 남아 계속 쓸 수 있습니다.',
      ],
      objection: '상을 받는다고 매출이 오르지는 않습니다. 다만 고객이 브랜드를 판단할 때 참고할 근거가 하나 늘어납니다.',
      ctas: [
        '접수 조건은 프로필 링크에서 확인하실 수 있습니다.',
        '우리 업종이 해당되는지 궁금하면 DM 주세요.',
        '준비 서류가 궁금하신 분은 댓글 남겨주세요.',
      ],
      /**
       * 쓰레드 전용 화법 — 공지가 아니라 '알게 된 걸 흘리는' 톤.
       * opens 는 톤 순서(신뢰/후킹/담백/축하)에 맞춰 4개.
       */
      threads: {
        opens: [
          '요즘 브랜드 인증 뭐가 있나 보다가 알게 된 건데요.',
          '이건 좀 의외였어요.',
          '연말에 몰리는 이유가 있더라고요.',
          '아는 분이 먼저 하고 알려주셔서 정리해 둡니다.',
        ],
        notes: [
          '방송사가 직접 여는 브랜드어워즈가 있더라고요. KBS N이랑 오픈엑스가 같이 하는 거라고.',
          '작년에만 139개 기업이 받았대요. 재작년은 108개였고.',
          '접수가 연중 상시라 시기 눈치 안 봐도 되는 게 편해 보이더라고요.',
          '엠블럼이랑 인증서만 주는 줄 알았는데 수상 기사에 인터뷰 영상까지 나온다고.',
          '원하면 KBS 드라마 자막광고에 2~3초 붙이는 것도 따로 있더라고요.',
          '올해 시상식은 12월 18일 상암이라고 합니다.',
        ],
        hedge: '물론 상 받는다고 매출이 뛰고 그러진 않죠. 고객이 판단할 때 볼 근거가 하나 생기는 정도.',
        closes: [
          '이런 거 챙기시는 분들 있을 것 같아서 적어둡니다.',
          '연말 정리하실 때 참고만 하세요.',
          '필요한 분들만 보시면 될 것 같아요.',
        ],
      },
    },
    topicPresets: [
      '2026 시상식 일정 안내 (12월 18일)',
      '기본 특전 6종 한눈에 정리',
      '슈퍼자막 패키지가 뭔가요',
      '수상 이력을 홍보에 활용하는 법',
    ],
  },
  {
    id: 'forbes',
    name: '포브스 브랜드어워즈',
    short: '포브스',
    icon: 'star',
    handle: '@forbesawards_openx',
    tagline: '중앙일보 주최 · 포브스코리아 주관',
    summary: '1,000점 기준 심사로 진행되며, 수상 시 중앙일보 연합광고와 포브스 특집 게재, 관련 포럼 초대가 이어집니다. 행사별로 일정과 분야가 다릅니다.',
    intake: '행사별 개별 접수',
    site: '',
    facts: [
      '1,000점 기준 심사',
      '수상 시 중앙일보 연합광고',
      '포브스 특집 게재 · 관련 포럼 초대',
    ],
    /**
     * 기준일(2026-08-13) 이후 일정만 '접수 가능'으로 표시
     * ⚠️ 주최·주관·후원이 행사마다 다르다. `host` 를 무시하고 "포브스코리아 주관"으로 뭉뚱그리면 틀린다.
     *    특히 E 는 포브스코리아가 '후원'이고 주관은 월간중앙 사무국이다.
     */
    events: [
      { code: 'A', name: '대한민국 신뢰받는 혁신대상', date: '2026-09-29', status: 'open',
        desc: '기술 혁신 · 품질 혁신 · 서비스 혁신 · 공공 혁신',
        host: '주최 중앙일보 · 주관 포브스코리아 사무국', place: '더 플라자호텔 다이아몬드홀' },
      { code: 'B', name: '소비자선정 최고의 브랜드 대상', date: '2026-02-27', status: 'closed',
        desc: '7개 부문군 · 온라인 소비자 투표 · JTBC 후원',
        host: '주최·주관 중앙일보, 포브스코리아 · 후원 JTBC', place: '' },
      { code: 'C', name: '고객신뢰도 1위 프리미엄 브랜드', date: '2026-10-30', status: 'open',
        desc: '4개 부문 · JTBC 후원',
        host: '주최·주관 중앙일보, 포브스코리아 · 후원 JTBC', place: '더플라자호텔' },
      { code: 'D', name: '대한상공회의소·포브스 사회공헌대상', date: '2026-05-27', status: 'closed',
        desc: '제17회 · ESG경영·CSV경영 중심 · 정부부처 후원',
        host: '주최 대한상공회의소, 중앙일보 · 주관 포브스코리아 · 후원 산업통상자원부, 보건복지부, JTBC',
        place: '더 플라자호텔' },
    ],
    benefits: ['중앙일보 연합광고', '포브스 특집 게재', '관련 포럼 초대'],
    packages: [],
    appeals: ['포브스 브랜드 권위', '중앙일보 보도', '기관 후원', 'ESG', '기업 신뢰도', '언론 활용'],
    cautions: [
      '공식 근거 없이 "대한민국 최고 권위"라고 단정하지 않음',
      '종료된 행사를 현재 접수 중인 것처럼 표현하지 않음',
      '행사별 주최·주관·후원기관을 혼동하지 않음 (사회공헌대상은 주최가 대한상공회의소·중앙일보다)',
      '구체 금액은 쓰지 않음 ("비용은 상담 시 안내" 문구는 허용)',
    ],
    /** ⚠️ 늘릴 때의 선은 kbsn 의 closings 주석 참고. 행사별로 일정·부문이 다르다는 점을 살렸다. */
    closings: [
      '수상 이력은 고객이 브랜드를 판단할 때 신뢰 근거로 활용될 수 있습니다.',
      '업종과 브랜드 상황에 따라 적합한 활용 방식이 달라질 수 있습니다.',
      '행사마다 일정과 부문이 다르니 어떤 부문이 맞는지부터 확인해 보시는 편이 좋습니다.',
      '중앙일보 연합광고와 포브스 특집 게재는 수상 이후 홍보 자료로 이어서 활용할 수 있습니다.',
      '수상 부문과 심사 기준을 함께 알리면 고객이 근거를 이해하기 쉬워집니다.',
    ],
    /** ⚠️ 첫 번째가 대표 태그다. 늘린 이유와 지켜야 할 선은 kbsn 의 hashtags 주석 참고. */
    hashtags: [
      '포브스브랜드어워즈', '포브스코리아', '중앙일보', '기업인증', 'ESG경영',
      'ESG', '사회공헌', '기업신뢰도', '브랜드마케팅', '기업브랜딩',
      '중소기업마케팅', '언론홍보', '기업홍보', '브랜드전략',
    ],
    voice: {
      hooks: [
        '1,000점 만점으로 심사합니다.',
        '"포브스 어워즈는 큰 기업만 받는 거 아닌가요?"',
        '올해 남은 일정은 두 개입니다. 9월 29일, 그리고 10월 30일.',
        '📢 하반기 포브스 브랜드어워즈 일정을 안내드립니다.',
      ],
      qa: [
        { q: '올해 남은 행사는 무엇인가요?',
          a: '9월 29일 대한민국 신뢰받는 혁신대상, 10월 30일 고객신뢰도 1위 프리미엄 브랜드 대상 두 개가 남아 있습니다.',
          shot: 'a desk calendar with two dates circled in red ink, a fountain pen resting beside it, morning light' },
        { q: '심사는 어떻게 진행되나요?',
          a: '1,000점을 기준으로 심사합니다. 중앙일보가 주최하고 포브스코리아가 주관합니다.',
          shot: 'a panel of judges reviewing documents around a long conference table, large windows, natural light' },
        { q: '수상하면 무엇이 이어지나요?',
          a: '중앙일보 연합광고와 포브스 특집 게재, 관련 포럼 초대가 이어집니다.',
          shot: 'a newspaper spread and an open business magazine side by side on a marble table' },
        { q: '행사마다 뭐가 다른가요?',
          a: '혁신대상은 기술·품질·서비스·공공 네 갈래로 나뉘고, 프리미엄 브랜드 대상은 4개 부문이며 JTBC가 후원합니다. 주최·주관·후원 기관이 행사마다 다르니 구분해서 보셔야 합니다.',
          shot: 'four separate spotlights illuminating four empty pedestals in a dark exhibition hall' },
        { q: '우리 업종도 해당되나요?',
          a: '행사별로 대상 분야가 정해져 있습니다. 혁신대상은 기술·품질·서비스·공공 영역을, 프리미엄 브랜드 대상은 소비자 접점이 있는 브랜드를 중심으로 봅니다.',
          shot: 'business people from several different industries standing together in a bright modern lobby' },
      ],
      shots: {
        cover: 'stacked business magazines on a marble table beside a window overlooking a city skyline, editorial still life',
        note: 'a business person standing at a tall window holding a document, weighing a decision, backlit',
        outro: 'an award trophy on a desk beside an open business magazine, evening city lights through the window',
      },
      /**
       * ⚠️ **재료가 적으면 주제를 바꿔도 같은 글이 나온다** (요청자 지적 2026-08-13).
       *    예전에는 5문장뿐이었다. 프롬프트가 "사실은 자료 안에서만"이라고 못박으니
       *    A·B·C 어떤 주제를 줘도 그 5문장으로 수렴했고, 장면(shot)까지 비슷해졌다.
       *    실제로 문제가 된 글은 이 5문장을 전부, 그것만 쓰고 있었다.
       *
       *    그래서 검증된 팩트시트(바탕화면 04_KNOWLEDGE_브랜드_정보_v2_통합.md)에서
       *    **주제가 갈릴 만한 축을 골라** 늘렸다 — 심사/배점, 부문, 서류, 홍보, 행사별 차이.
       *    주제가 달라지면 모델이 **다른 묶음**을 집어 갈 수 있어야 한다.
       * ⚠️ 늘릴 때도 팩트시트에 있는 사실만 쓴다. 지어내면 사실성 원칙 위반이다.
       */
      proof: [
        // 주최·주관 (행사마다 다르다)
        '중앙일보가 주최하고 포브스코리아가 주관합니다.',
        '사회공헌대상은 대한상공회의소와 중앙일보가 주최하고 산업통상자원부, 보건복지부, JTBC가 후원합니다.',
        '소비자선정 최고의 브랜드 대상과 고객신뢰도 1위 프리미엄 브랜드는 JTBC가 후원합니다.',
        // 심사·배점
        '심사는 1,000점을 기준으로 진행됩니다.',
        '혁신대상은 리더십 300점, 혁신 성과 500점, 사회적 책임 200점으로 나눠 심사합니다.',
        '공공혁신 부문은 리더십 150점, 경영 및 조직 300점, 혁신 성과 400점, 사회적 책임 150점입니다.',
        '사회공헌대상은 사회적 책임 300점, 사회공헌 활동 400점, 사회공헌 성과 300점으로 심사합니다.',
        '소비자선정 최고의 브랜드 대상은 부문별 온라인 소비자 투표를 거칩니다.',
        // 부문
        '9월 29일 대한민국 신뢰받는 혁신대상은 기술·품질·서비스·공공 네 갈래로 나뉩니다.',
        '기술혁신 부문은 정보통신, 에너지, 화학, 환경, 자동차, 신소재, 바이오, 식품, 의료, 화장품 등을 다룹니다.',
        '서비스혁신 부문은 금융, 유통, 교육, 의료, 법률, 정보통신, 관광, 보안, 공공행정 등이 대상입니다.',
        '10월 30일 고객신뢰도 1위 프리미엄 브랜드 대상은 4개 부문이며 JTBC가 후원합니다.',
        '프리미엄 브랜드 대상의 4개 부문은 기업·서비스, 패션·유통·프랜차이즈, 식품·제조·건설, 관광·농특산물입니다.',
        '사회공헌대상은 ESG경영과 CSV경영을 대상으로 하고, 지역사회공헌·의료봉사·문화예술 등은 연속상으로 시상합니다.',
        // 접수·서류
        '응모신청서와 사업자등록증 사본, 공적기술서를 사무국에 제출합니다.',
        '사회공헌대상은 공적기술서를 사회공헌 백서나 지속가능경영보고서로 대신할 수 있습니다.',
        '선정된 기업은 기업홍보물을 추가로 제출합니다.',
        // 수상 후
        '수상하면 중앙일보 연합광고와 포브스 특집 게재, 관련 포럼 초대가 이어집니다.',
        '포브스코리아 특집기사와 포털 온·오프라인 홍보가 함께 진행됩니다.',
        '세미나와 포럼에 강연자로 초대되기도 합니다.',
        // 기대효과
        '사회적 이미지와 대외 위상을 높여 경쟁력을 개선하는 데 쓰입니다.',
        '내부 임직원의 동기부여와 조직 결속에도 활용됩니다.',
        // 진행 과정
        '사전조사로 후보 기업을 고른 뒤 응모와 공적서 접수가 이어집니다.',
        '각계 전문가로 구성된 심사위원이 공적서를 서류심사합니다.',
        '부문별로 선정한 뒤 기업에 개별 통지합니다.',
        '사회공헌대상은 산·학·연 전문가 심사 후 최종심사위원회를 따로 구성합니다.',
        // 부문 세부
        '품질혁신 부문은 은행, 보험, 건설, 패션, 정보통신, 식품, 에너지, 환경 등을 봅니다.',
        '공공혁신 부문은 일자리창출, 상생협력, 공익안전, 중소기업지원 등을 다룹니다.',
        '소비자선정 최고의 브랜드 대상은 금융, 가전, 유통·패션, 통신·교육, 식품, 의료, 공공행정으로 나뉩니다.',
        '사회공헌대상 연속상은 지역사회공헌, 의료봉사, 문화예술, 일자리창출 등으로 나뉩니다.',
        // 평가 항목
        '리더십 항목은 경영철학과 비전, 혁신 방향과 의지를 봅니다.',
        '혁신 성과 항목은 혁신 사례와 경쟁력 기여도, 전문 인재양성을 봅니다.',
        '사회적 책임 항목은 사회공헌 활동과 수상 경력, 이미지 향상 기여도를 봅니다.',
        '프리미엄 브랜드 대상은 고객관리 체계와 소비자 권익보호를 함께 봅니다.',
        // 기대효과
        '브랜드 인지도를 높이고 긍정적인 연상을 강화하는 데 씁니다.',
        '소비자와의 신뢰 구축과 지역사회 유대 강화에도 활용됩니다.',
        '지속가능경영에 대한 투자 매력을 높이는 근거가 되기도 합니다.',
        '존경받는 기업이라는 국민적 공감대를 만드는 데 도움이 됩니다.',
      ],
      objection: '권위를 말로 설명하기보다, 주최·주관과 심사 기준을 직접 확인해 보시는 편이 빠릅니다. 행사마다 일정과 대상 분야가 다르니 구분해서 보셔야 합니다.',
      ctas: [
        '행사별 접수 일정은 프로필 링크에서 확인하실 수 있습니다.',
        '어느 행사가 맞을지 궁금하면 DM 주세요.',
        '부문 구분이 헷갈리시면 댓글로 물어보셔도 됩니다.',
      ],
      threads: {
        opens: [
          '남은 일정 찾아보다가 정리하게 됐어요.',
          '포브스 어워즈가 하나인 줄 알았는데 아니더라고요.',
          '심사 방식이 생각보다 구체적이던데요.',
          '이건 헷갈리는 분들 많을 것 같아서 남겨둡니다.',
        ],
        notes: [
          '중앙일보가 주최하고 포브스코리아가 주관하는 구조더라고요.',
          '1,000점 기준으로 심사한다고 합니다.',
          '올해 남은 건 9월 29일이랑 10월 30일, 두 개예요.',
          '9월 건 기술·품질·서비스·공공으로 나뉘고, 10월 건은 4개 부문에 JTBC 후원이라고.',
          '수상하면 중앙일보 연합광고랑 포브스 특집까지 이어진다고 하더라고요.',
        ],
        hedge: '행사마다 주최랑 후원이 다 달라서, 이름만 보고 같은 거라고 생각하면 헷갈리기 쉬워요.',
        closes: [
          '알아보시는 분들 참고하시라고 적어둡니다.',
          '둘 중에 뭐가 맞을지는 업종 보고 판단하셔야 할 것 같아요.',
          '저도 정리하면서 알게 된 부분이라 남겨둡니다.',
        ],
      },
    },
    topicPresets: [
      '신뢰받는 혁신대상 9월 29일 안내',
      '프리미엄 브랜드 대상 4개 부문 소개',
      '1,000점 심사 기준은 어떻게 되나요',
      '수상 후 언론 노출 활용법',
    ],
  },
  {
    id: 'kcst',
    name: '대한민국 고객만족 신뢰도 대상',
    short: 'KCST',
    icon: 'shield',
    handle: '@kcstawards_official',
    tagline: '전 업종 신청 가능 · 연중 상시 접수',
    summary: '소상공인과 중소기업도 접근할 수 있는 구조이며 비대면 시상이 가능합니다. 매장과 온라인 채널에서 바로 쓰는 인증물이 함께 제공됩니다.',
    intake: '연중 상시 접수',
    site: 'https://kcsta.co.kr/',
    /** ⚠️ 3개 기관을 항상 함께 표기한다. 하나만 쓰면 틀린다. */
    host: '주최 대한민국고객만족평가원(주) · 주관 대한민국소비자신문 · 후원 한국인터넷신문방송기자협회',
    facts: [
      '영문 표기 · Korea Customer Satisfaction & Trust Awards',
      '전 업종 신청 가능',
      '비대면 시상 가능',
      '소상공인·중소기업 친화 구조',
    ],
    /** 심사 기준 — 합계 100% 를 정확히 유지할 것 */
    criteria: [
      { label: '고객만족도', weight: 30 },
      { label: '브랜드 신뢰도', weight: 25 },
      { label: '품질', weight: 20 },
      { label: '시장 반응', weight: 15 },
      { label: '사회적 책임', weight: 10 },
    ],
    benefits: ['상장', '상패', '인증서', '네이버 플레이스 배너', 'X배너', '메탈 현판', '언론 기사 배포'],
    packages: [],
    appeals: ['전 업종 참여', '연중 상시 접수', '소상공인 친화', '매장·온라인 활용 인증물'],
    cautions: [
      '구체 금액은 쓰지 않음 — "저렴한"·"가성비"처럼 가격을 주장하는 표현도 금지 ("상담 시 안내"는 허용)',
      '언론 노출은 "200여 개 제휴사 동시 송출"로 표기 (매체 200개 / 기사 200개 아님)',
      '심사 기준의 합계와 비율을 정확하게 유지',
      '주최·주관·후원 3개 기관을 함께 표기',
    ],
    /** ⚠️ 늘릴 때의 선은 kbsn 의 closings 주석 참고. **비용은 한 글자도 꺼내지 않는다**(4절). */
    closings: [
      '고객 신뢰와 브랜드 품질을 보여주는 객관적 근거로 활용될 수 있습니다.',
      '업종과 브랜드 상황에 따라 적합한 활용 방식이 달라질 수 있습니다.',
      '상장과 인증물은 매장과 온라인 채널 양쪽에서 바로 쓸 수 있습니다.',
      '전 업종이 신청할 수 있어 우리 업종이 되는지부터 고민하지 않아도 됩니다.',
      '심사 기준을 먼저 보면 무엇을 준비해야 할지 정리됩니다.',
    ],
    /** ⚠️ 첫 번째가 대표 태그다. '네이버플레이스'는 특전(플레이스 배너)에 근거가 있다. */
    hashtags: [
      'KCST', '고객만족도대상', '기업인증', '브랜드신뢰도', '소상공인',
      '고객만족', '소상공인마케팅', '자영업자', '매장마케팅', '동네가게',
      '브랜드마케팅', '기업홍보', '인증마크', '네이버플레이스',
    ],
    voice: {
      hooks: [
        '심사 항목 중 고객만족도가 30%를 차지합니다.',
        '"소상공인도 신청할 수 있나요?" — 전 업종 가능합니다.',
        '연중 상시 접수, 비대면 시상도 됩니다.',
        '📢 KCST 대한민국 고객만족도 신뢰도 대상, 상시 접수 중입니다.',
      ],
      qa: [
        { q: '어떤 기준으로 심사하나요?',
          a: '고객만족도 30%, 브랜드 신뢰도 25%, 품질 20%, 시장 반응 15%, 사회적 책임 10%로 심사합니다.',
          shot: 'a clipboard with a checklist and a pen on a clean desk, soft daylight from the side' },
        { q: '무엇을 받게 되나요?',
          a: '상장과 상패, 인증서에 더해 네이버 플레이스 배너, X배너, 메탈 현판이 나옵니다. 언론 기사 배포도 특전에 포함됩니다.',
          shot: 'a metal certification plaque, a rolled certificate and a standing banner arranged inside a small shop' },
        { q: '작은 매장도 신청할 수 있나요?',
          a: '전 업종이 신청할 수 있습니다. 심사가 매출 규모가 아니라 고객만족도와 브랜드 신뢰도 중심이라 소상공인도 접근할 수 있는 구조입니다.',
          shot: 'a small cafe storefront on a quiet neighborhood street, warm interior lights, welcoming' },
        { q: '시상식에 꼭 가야 하나요?',
          a: '비대면 시상이 가능합니다. 일정을 비우기 어려운 곳도 참여할 수 있습니다.',
          shot: 'a laptop on a shop counter showing a video call, small store interior blurred behind' },
        { q: '받은 다음에는 어디에 쓰나요?',
          a: '메탈 현판은 매장 입구에, X배너는 상담 공간이나 행사장에 둡니다. 네이버 플레이스 배너는 손님이 온라인에서 가게를 찾을 때 바로 보입니다.',
          shot: 'a shop entrance with a certification plaque beside the door and a smartphone held up showing a map listing' },
      ],
      shots: {
        cover: 'a warm neighborhood shop interior at golden hour, owner smiling behind the counter, inviting atmosphere',
        note: 'a small shop owner standing outside their storefront with arms crossed, looking at the sign, thinking',
        outro: 'a bright shop entrance with a certification plaque beside the door, a customer walking in',
      },
      proof: [
        // ⚠️ 재료가 적으면 주제를 바꿔도 같은 글이 나온다 — forbes proof 주석 참고
        // 주최
        '대한민국고객만족평가원이 주최하고 대한민국소비자신문이 주관하며 한국인터넷신문방송기자협회가 후원합니다.',
        // 심사 (항목별로 따로 쓸 수 있게 쪼갬)
        '고객만족도 30%, 브랜드 신뢰도 25%, 품질 20%, 시장 반응 15%, 사회적 책임 10%로 심사합니다.',
        '고객만족도는 제품·서비스 만족도와 고객 응대, 재구매·추천 의사를 봅니다.',
        '브랜드 신뢰도는 브랜드 이미지와 윤리성, 소비자 신뢰 지수를 봅니다.',
        '품질 경쟁력은 제품·서비스 품질과 인증·특허·기술력, 품질의 지속성을 봅니다.',
        '시장 반응은 매출 성장률과 SNS·미디어 노출, 온라인 리뷰를 함께 봅니다.',
        '사회적 책임은 ESG·친환경 경영과 지역사회 공헌, 윤리 경영 활동을 봅니다.',
        '전문 심사위원단이 종합 평가한 뒤 최종 수상 브랜드를 선정합니다.',
        // 자격·절차
        '전 업종이 신청할 수 있고 접수는 연중 상시로 열려 있습니다.',
        '소상공인과 중소기업, 브랜드, 서비스업까지 폭넓게 참여할 수 있습니다.',
        '참가신청서와 사업자등록증을 제출하면 됩니다.',
        '참가 신청, 심사 진행, 수상 확정, 특전 발송 네 단계로 진행됩니다.',
        '특전은 제작이 끝나면 등기로 발송됩니다.',
        '성인, 도박 등 불법적이거나 사행성을 조장하는 업종은 신청할 수 없습니다.',
        // 특전 (쓰임새가 다 다르다)
        '수상하면 상장과 상패, 인증서에 더해 네이버 플레이스 배너와 X배너, 메탈 현판이 함께 나옵니다.',
        '메탈 현판은 로비나 매장에 걸어 방문 고객에게 바로 보여줄 수 있습니다.',
        '네이버 플레이스 배너는 손님이 온라인에서 가게를 찾을 때 노출됩니다.',
        'X배너는 매장이나 행사장에 세워 쓸 수 있습니다.',
        '온·오프라인에서 쓸 수 있는 엠블럼 이미지도 함께 제공됩니다.',
        // 언론
        '언론 기사 배포도 특전에 포함됩니다.',
        '대한민국 소비자신문 기사가 200여 개 제휴사로 동시 송출됩니다.',
        '수상이 확정되면 대한민국 소비자신문에 기사가 자동으로 보도됩니다.',
        // 기타
        '비대면 시상이 가능해 일정을 비우기 어려운 곳도 참여할 수 있습니다.',
        '푸른별 어린이 재단 후원이 연계되어 있습니다.',
        '후원은 위기가정 긴급지원, 아동 의료·건강, 교육·정서지원, 주거환경 개선에 쓰입니다.',
        // 활용 효과
        '공신력 있는 인증으로 경쟁 브랜드와의 차별화 포인트를 만들 수 있습니다.',
        '인증 이미지와 배너를 SNS나 온라인 광고에 넣어 신뢰도를 높입니다.',
        '보도된 기사가 검색 결과에 남아 브랜드 노출로 이어집니다.',
        '매장을 찾은 손님에게 수상 사실을 바로 보여줄 수 있습니다.',
        // 실물
        '상패와 트로피, 상장이 한 상자에 담겨 등기로 옵니다.',
        '상패에는 부문과 기업명, 브랜드명이 함께 새겨집니다.',
        '영문 표기는 Korea Customer Satisfaction & Trust Awards 입니다.',
        // 부문 예시 (업종이 넓다는 근거)
        '화환, 강아지매트, 붙박이장, 맞춤정장처럼 부문이 잘게 나뉘어 있습니다.',
        '아파트 사전점검이나 치과의료서비스 같은 서비스 부문도 있습니다.',
        '마케팅기획, 화장품 연구개발처럼 업종이 아닌 기능으로 나뉜 부문도 있습니다.',
        '지역별로도 우수 사업장을 나눠 선정합니다.',
        'CEO 개인을 대상으로 하는 부문도 있습니다.',
      ],
      objection: '규모가 작으면 어렵지 않을까 걱정하시는 경우가 많습니다. 심사는 매출 규모가 아니라 고객만족도와 브랜드 신뢰도 중심으로 이뤄집니다.',
      ctas: [
        '신청 절차는 프로필 링크에서 보실 수 있습니다.',
        '우리 업종이 되는지 궁금하면 DM 주세요.',
        '현판·배너 실물이 궁금하시면 댓글 남겨주세요.',
      ],
      threads: {
        opens: [
          '심사 기준 보고 좀 놀랐어요.',
          '작은 가게 하시는 분들이 의외로 모르시더라고요.',
          '이건 생각보다 문턱이 낮았습니다.',
          '매장에 걸 만한 게 없나 찾다가 봤는데요.',
        ],
        notes: [
          '심사에서 고객만족도가 30%로 제일 크더라고요. 그다음이 브랜드 신뢰도 25%.',
          '전 업종이 신청 가능하고, 접수도 연중 상시로 열려 있더라고요.',
          '비대면 시상도 된다고 해서 일정 부담은 덜하겠다 싶었어요.',
          '상장이랑 상패만 주는 게 아니라 메탈 현판, X배너, 네이버 플레이스 배너까지 나온다고.',
          '언론 기사 배포도 포함이라고 하더라고요.',
        ],
        hedge: '싸다고 말하긴 좀 그렇고요. 다만 매출 규모로 거르는 구조가 아니라는 게 포인트인 것 같아요.',
        closes: [
          '동네에서 장사하시는 분들 참고하시면 좋을 것 같아 남깁니다.',
          '플레이스 배너 하나만 봐도 나쁘지 않더라고요.',
          '필요하신 분들만 보세요.',
        ],
      },
    },
    topicPresets: [
      '심사 기준 5가지 비중 공개',
      '특전 7종 정리 (현판·배너 포함)',
      '소상공인도 신청할 수 있나요',
      '네이버 플레이스 배너 활용법',
    ],
  },
  {
    id: 'aitvcf',
    name: '중소기업 AI TV CF',
    short: 'AI TV CF',
    icon: 'tv',
    handle: '@aitvcf_official',
    tagline: 'TV CF 대중화 선언',
    summary: 'AI 기반 CF 제작부터 IPTV 송출까지 연결되는 상품입니다. 300만원 상당 AI CF를 무상으로 제작하고, IPTV 3사 약 90개 채널에 송출합니다. 지역 타겟팅과 완전 시청 기준 과금을 활용할 수 있고 매월 송출 결과 리포트가 나옵니다.',
    intake: '상담 후 진행',
    site: '',
    facts: [
      '300만원 상당 AI CF 무상지원',
      'IPTV 3사 · 2025년 12월 기준 3,174만 가입가구',
      '약 90개 채널 송출 · 채널 광고 사이 120초 편성',
      '시·군·구 단위 지역 타겟팅 · 완전 시청 기준 과금',
      '방송 심의 규정 준수 제작 · 매월 송출 결과 리포트',
    ],
    benefits: ['300만원 상당 AI CF 무상 제작', '언론 보도자료 무상 송출(200여 개 제휴사)', '브랜드대상 수상권(장기계약 시)', 'IPTV 광고 송출', '매월 송출 결과 리포트'],
    packages: [],
    appeals: ['TV 매체 신뢰도', '제작비 부담 완화', '지역 단위 타겟 광고', '온라인 광고와 다른 신뢰 접점'],
    targets: [
      '지역 인지도를 높이고 싶은 브랜드',
      '신제품 또는 매장 오픈을 알리고 싶은 기업',
      '온라인 광고 이외의 접점이 필요한 브랜드',
      '영상 제작과 송출을 함께 진행하고 싶은 기업',
    ],
    cautions: [
      "공식 표기는 '300만원 상당 AI CF 무상지원' — 체감은 세게 쓰되 '지원금·드립니다·할인'으로 바꾸지 않음",
      "제작비 소거형 카피는 허용 — 'TV CF 제작비 0원' '300만원 → 0원' '제작비는 저희가 냅니다'",
      '정부·국가 지원사업으로 오인될 표현을 쓰지 않음 (오픈엑스 자체 프로모션이다)',
      '광고 성과를 무조건 보장하지 않음',
      '전국 송출과 지역 타겟 송출을 혼동하지 않음',
      '브랜드대상 수상권은 "※ 장기계약 시 적용"을 병기',
    ],
    /**
     * ⚠️ 늘릴 때의 선은 kbsn 의 closings 주석 참고. 이 상품은 특히 조심한다 —
     *    **광고 성과를 암시하지 않고, '지원금·드립니다·할인'으로 바꾸지 않는다**(4절).
     *
     * ⚠️ 규제하는 건 **단어**지 **임팩트**가 아니다 (2026-08-14).
     *    독자가 '300만원 지원받는다 / 할인받는다'와 같은 크기로 체감하게 쓰되, 그 두 단어만 피한다.
     *    '지원금'은 정부 현금으로 오인되고, '할인'은 정가가 있다는 전제를 만들어
     *    "그럼 원래 얼마인데요"로 대화가 끌려간다 — 비공개인 집행 단가 질문이 상담 앞으로 나온다.
     *    '제작 지원'·'무상 제작'은 사실이므로 쓴다.
     */
    closings: [
      'AI 기반 제작과 IPTV 송출을 통해 기존보다 접근 가능한 TV 광고 방식을 검토할 수 있습니다.',
      '업종과 브랜드 상황에 따라 적합한 활용 방식이 달라질 수 있습니다.',
      '지역과 채널을 어디까지 좁힐지부터 정하면 나머지 결정이 쉬워집니다.',
      '매월 나오는 송출 결과 리포트로 광고가 어떻게 나갔는지 확인할 수 있습니다.',
      '온라인 광고와 다른 접점이 필요한지부터 따져 보시면 판단이 빨라집니다.',
    ],
    /** ⚠️ 첫 번째가 대표 태그다. **'무료·지원금' 계열 태그를 넣지 말 것** — 정부 지원으로 오인된다. */
    hashtags: [
      '중소기업광고', 'AITVCF', 'IPTV광고', 'TV광고', '지역광고',
      '티비광고', '광고제작', 'AI광고', 'AI영상', '영상마케팅',
      '중소기업마케팅', '매장홍보', '브랜드광고', '광고집행',
    ],
    voice: {
      hooks: [
        '300만원짜리 TV CF를 0원에 만듭니다.',
        '"TV 광고는 대기업만 하는 거 아닌가요?"',
        '중소기업이 TV 광고를 못 하는 이유는 딱 하나, 제작비입니다.',
        '📢 중소기업 AI TV CF, 제작부터 송출까지 안내드립니다.',
      ],
      qa: [
        { q: 'AI로 만든 CF는 어떻게 나오나요?',
          a: 'AI 기반으로 CF를 제작하고 그대로 IPTV 송출까지 연결됩니다. 제작 업체와 송출 매체를 따로 알아보지 않아도 됩니다.',
          shot: 'a video editing workspace, wide monitor showing a timeline, warm studio lighting' },
        { q: '어디에 송출되나요?',
          a: 'IPTV 3사 기반 채널을 통해 송출됩니다. 채널 수와 송출 범위는 상품 조건에 따라 달라지니 최신 자료를 확인해 주세요.',
          shot: 'three television screens side by side in a dark room, each showing a different broadcast' },
        { q: '지역만 골라서 틀 수 있나요?',
          a: '지역 단위 타겟팅이 가능합니다. 다만 전국 송출과 지역 타겟 송출은 조건이 다르니 어느 쪽인지 확인하고 진행하셔야 합니다.',
          shot: 'an aerial night view of a city district, glowing streets forming a map-like pattern' },
        { q: '광고비는 어떻게 계산되나요?',
          a: '완전 시청을 기준으로 과금하는 방식을 활용할 수 있습니다. 구체적인 금액과 조건은 상품 자료에서 확인해 주세요.',
          shot: 'a desk with a calculator and an open notebook, a small television glowing in the background' },
        { q: '어떤 브랜드에 맞나요?',
          a: '지역 인지도를 높이려는 브랜드, 신제품이나 매장 오픈을 알리려는 기업, 온라인 광고 외에 다른 접점이 필요한 곳에서 주로 검토합니다.',
          shot: 'a newly opened store with a grand opening ribbon across the entrance, bright daylight' },
      ],
      shots: {
        cover: 'a living room television glowing in a dim room playing a commercial, family silhouettes on the sofa',
        note: 'a small business owner watching television alone in a quiet shop after closing hours',
        outro: 'a storefront sign glowing at night, a television screen visible through the shop window',
      },
      proof: [
        // ⚠️ 재료가 적으면 주제를 바꿔도 같은 글이 나온다 — forbes proof 주석 참고
        // 오퍼
        '300만원 상당의 AI CF를 무상으로 제작해 드립니다.',
        '언론 보도자료 송출과 브랜드대상 수상권도 함께 지원됩니다.',
        '브랜드대상 수상권은 장기계약 시 적용됩니다.',
        // 매체 규모
        'IPTV는 2025년 12월 기준 3,174만 가구가 쓰는 TV 플랫폼입니다.',
        'IPTV 3사 기반 채널을 운영하고 있어 시·군·구 단위 지역 타겟팅이 가능합니다.',
        'KT, B tv, U+tv 세 곳에 모두 내보낼 수도 있고 상품별로 골라서 내보낼 수도 있습니다.',
        'tvN, JTBC, 채널A, MBN, TV조선, YTN, 연합뉴스TV 등 약 90개 채널에 송출됩니다.',
        '광고는 프로그램 사이 120초 편성 구간에 들어갑니다.',
        // 시청자 특성
        'IPTV 가입가구는 30~50대 비중이 절반을 차지합니다.',
        'TV 플랫폼 특성상 가구 내 동반 시청자가 평균 1.9명입니다.',
        // 타겟팅·과금
        '100% 완전히 시청된 광고 노출에 한해서만 과금됩니다.',
        '가입자 정보를 바탕으로 시·군·구 단위까지 지역을 좁힐 수 있습니다.',
        '인기 채널이나 타겟이 주로 보는 채널을 골라 노출할 수 있습니다.',
        'TV 시청이력과 모바일 데이터를 활용한 관심사 기반 타겟팅도 가능합니다.',
        // 제작·운영
        'AI로 CF를 만들고 IPTV로 내보내는 과정이 하나로 연결됩니다.',
        '촬영 장비와 스태프 없이 기획과 AI 기술만으로 영상을 만듭니다.',
        '재촬영 없이 프롬프트 수정만으로 날씨나 배경, 모델을 바꿀 수 있습니다.',
        '방송 심의 규정을 준수해 제작하므로 심의 걱정이 없습니다.',
        '매월 송출 결과 리포트를 제공합니다.',
        '상담부터 전략 제안, 제작, 송출, 월별 리포트, 후속 마케팅까지 이어집니다.',
        // 사례
        '소상공인 브랜딩 광고부터 지역 음식점 로컬광고까지 다양하게 송출되고 있습니다.',
        '동물병원, 헬스장, 프랜차이즈 음식점, 안과 같은 업종에서 제작한 사례가 있습니다.',
        '신제품을 알리거나 매장을 새로 여는 시점에 검토하는 경우가 많습니다.',
        // IPTV 매체 강점
        'TV 광고의 규모감과 디지털 광고의 효율을 함께 가져갑니다.',
        '검증된 콘텐츠 사이에 붙어 브랜드 안전성이 확보됩니다.',
        '광고 효과를 데이터로 확인할 수 있습니다.',
        'IPTV는 지상파와 케이블 100여 개 채널에 방송·영화·키즈 VOD까지 함께 제공합니다.',
        // 3사 규모
        'KT는 1,300만 명, B tv는 660만 명, U+tv는 540만 명 규모입니다.',
        'KT 상품과 B tv·U+tv 결합 상품 중에서 고를 수 있습니다.',
        '한 곳에서 IPTV 3사 송출을 한 번에 진행할 수 있습니다.',
        // 시청자
        'IPTV 가입가구는 서울·경기·인천이 45%를 차지합니다.',
        '월 소득 300만원대 가구가 33%로 가장 많습니다.',
        // 제작
        '기존 TV CF와 달리 촬영 스튜디오나 모델 섭외가 필요 없습니다.',
        '제작 신청 후 담당자가 배정되고 피드백을 거쳐 완성됩니다.',
        '제품을 앞세운 영상, 모델을 앞세운 영상, 이야기를 앞세운 영상 모두 만듭니다.',
        '봉안당이나 자동차 세차처럼 흔치 않은 업종의 사례도 있습니다.',
      ],
      objection: '광고를 튼다고 성과가 보장되지는 않습니다. 다만 온라인 광고와는 성격이 다른 신뢰 접점을 하나 더 만드는 방법으로 검토해 볼 수 있습니다.',
      ctas: [
        '송출 범위와 조건은 프로필 링크에서 확인해 주세요.',
        '우리 지역이 되는지 궁금하면 DM 주세요.',
        '업종별 심의 가능 여부는 댓글로 문의 주세요.',
      ],
      threads: {
        opens: [
          '광고비 계산 방식이 생각보다 합리적이던데요.',
          'TV 광고는 아직도 대기업만 하는 건 줄 알았는데요.',
          '지역만 골라서 튼다는 게 좀 신기했어요.',
          '이거 모르시는 분들 꽤 많을 것 같아서 적어둡니다.',
        ],
        notes: [
          'AI로 CF 만들고 IPTV로 내보내는 걸 한 번에 해주더라고요.',
          'IPTV 3사 기반 채널이라 지역 단위로 골라서 송출된다고 합니다.',
          '완전 시청 기준으로 과금하는 방식도 쓸 수 있다고 하더라고요.',
          '제작 따로 송출 따로 알아볼 필요가 없는 게 제일 편해 보였어요.',
          '매장 새로 열거나 신제품 낼 때 많이들 본다고 하더라고요.',
        ],
        hedge: '물론 튼다고 바로 효과가 나오는 건 아니고요. 온라인 광고랑은 접점 성격이 다르다는 정도로 보시면 될 것 같아요.',
        closes: [
          '온라인 광고만 하다 지치신 분들 참고하시라고요.',
          '지역에서 장사하시는 분들한테 특히 맞을 것 같더라고요.',
          '필요한 분들만 챙겨가세요.',
        ],
      },
    },
    topicPresets: [
      '우리 브랜드도 TV에 나옵니다',
      'IPTV 지역 타겟팅이란',
      '완전 시청 기준 과금 방식 설명',
      '매장 오픈을 TV로 알리는 법',
    ],
  },
];

/**
 * 공통 금지 표현 — 카피 작성 화면에서 경고로 노출한다
 * ⚠️ 어형 변화까지 잡는 것은 `lib/copywriter.js` 의 `RISKY` 정규식이다. 여기는 정확히 일치하는 문구만.
 */
export const BANNED_PHRASES = [
  '수상하면 매출이 무조건 상승',
  '모든 기업이 반드시',
  '100% 보장',
  '가장 권위 있는',
  '신청만 하면 수상',
  // ⚠️ 2026-08-14 개정 — '참가비'·'심사비'·'비용은 상담'을 금칙어에서 뺐다.
  //    막는 것은 금액이지 주제가 아니다. "비용은 상담 시 안내드립니다"는 이제 허용한다.
  //    대신 **가격을 주장하는 표현**을 막는다 — 이쪽이 표시광고 리스크다.
  '저렴한',
  '가성비',
  '부담 없는 비용',
  '부담 없는 가격',
  // AI TV CF — 현금성·정부지원 오인 표현. 사실은 '300만원 상당 AI CF 무상지원'이다.
  // 체감은 세게 써도 되지만(제작비 0원 / 300만원 → 0원) 아래 단어로 바꾸면 안 된다.
  '300만원 지원금',
  '300만원 드립니다',
  '300만원 할인',
  '300만원 캐시백',
  '정부 지원금',
];

/** 채널 정의 — 2단계(아이디어 문서화)에서 사용 */
export const CHANNELS = [
  // ⚠️ 분량은 **카드 장수에 따라 달라진다.** 실제 목표치는 `lib/copywriter.js` 의 `BLOG_PLAN`
  //    한 곳에만 있고(6장 1,200~1,600 … 1장 700~1,000), 규칙 기반과 AI 프롬프트가 같은 표를 본다.
  //    여기 `limit` 은 그 위에 두는 **안전망**이다 — 캡션·개요표가 붙어도 넘지 않을 값.
  //    예전엔 1,800~2,400자 고정이라 1장을 골라도 긴 글이 나왔다(요청자 지적 2026-08-13).
  { id: 'blog', name: '블로그', icon: 'blog', hint: '검색 유입 중심 · 장수에 맞춰 700~1,600자 + 이미지 + 캡션',
    // ⚠️ limitLabel 은 **본문 기준**이다 (2026-08-14). 📷·⤷·🔔·해시태그는 세지 않는다 —
    //    `BLOG_PLAN`(lib/copywriter.js)·`bodyLength()`·화면 카운터가 모두 같은 정의를 본다.
    limit: 2000, limitLabel: '본문 기준 장수에 따라 600~1,100자' },
  { id: 'instagram', name: '인스타그램', icon: 'instagram', hint: '첫 두 줄 후킹 · 해시태그 · 카드뉴스 연계',
    // ⚠️ 인스타 자체 상한은 2,200자지만 **그만큼 쓰면 아무도 안 읽는다**(요청자 지적 2026-08-13).
    //    인기 정보 계정 캡션은 400자 안팎이다. 프롬프트는 500자를 목표로 지시하고
    //    여기 limit 은 편집 여유를 둔 안전망이다.
    limit: 700, limitLabel: '권장 400~500자' },
  // ⚠️ 500 → 300 (2026-08-14, 요청자 지시: "쓰레드는 500자에서 300자로 줄이자. 글이 너무 많다").
  //    쓰레드는 원래 '한 가지만 흘리는' 채널이라 길면 톤 자체가 무너진다.
  //    ⚠️ 여기만 고치면 안 된다 — `threadsCopy()`(규칙 기반)와 `channelRules()`(AI 프롬프트)의
  //       숫자도 같이 본다. 세 곳이 어긋나면 경로마다 다른 길이가 나온다.
  { id: 'threads', name: '쓰레드', icon: 'thread', hint: '짧은 대화체 · 300자 제한 · 질문형 마무리',
    limit: 300, limitLabel: '최대 300자' },
];

/** @param {ProductId} id */
export const getProduct = (id) => PRODUCTS.find((p) => p.id === id) || null;
