...
| 템플릿 선택·강조 색상 | localStorage | 3단계에서 다시 선택 |
...
components/           header · stepper · product-card · toast · imagepanel
pages/                home · copy · template · library
```
...
`pages/library.js` 가 아직 자리표시자다. 검색·필터·정렬을 만든다.
그다음이 PART 2(백엔드)다. PART 2에서는 이미지 생성 호출을 서버 프록시로 옮기고
API 키를 서버에만 둔다.
