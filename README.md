...
어워즈형은 상패·왕관·메달 같은 심볼을 프롬프트가 직접 지어낸다.
마케터형은 사람 사진이 아니라 **기하학적 마크**를 만든다. 아바타는 목록에서 40px 로도 뜨는데
인물 사진은 그 크기에서 뭉개져 계정이 구분되지 않기 때문이다.

> **로고를 넣어 만드는 기능은 화면에서 뺐다.** 브랜드 로고 파일이 아직 없어서
> 빈 칸만 자리를 차지했다. 생성 쪽 코드는 남아 있으므로 로고가 준비되면 다시 붙인다.
...
| 채널 | 구조 |
|---|---|
| 블로그 | 제목 → **3줄 요약** → 목차 → Q&A 소제목 → 남은 일정 → 반론 → 정리. 문단 사이에 `📷 [이미지 N · 역할]`로 **카드뉴스가 들어갈 자리**를 표시한다. 본문 1,400~2,000자 · 기본 핵심 소제목 3개 — **카드 장수와 무관하다**. 주제에 항목 수가 명시되면 그 수를 따른다. |
...
- **테마 색상 (카드형)** — 파랑·초록·노랑·보라·자주·먹색 + 직접 입력(hex).
  직접 넣은 색이 흰 글씨와 대비가 모자라면 입력칸 아래에 몇 대 몇인지 알려 준다(막지는 않는다).
