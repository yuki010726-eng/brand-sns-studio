...
function fallbackBg(g, conceptId, L, theme, paper, grain) {
  if (conceptId === 'note') {
     ...
     */
    g.fillStyle = paper.hex;
    g.fillRect(0, 0, W, H);
    if (grain.dots) {
      g.fillStyle = `rgba(17,17,17,${grain.alpha})`;
      for (let i = 1; i <= grain.dots; i++) {
...
function drawNoteSymbol(g, id, { x, y, size, color }) {
  ...
  if (sym.id === 'flask') {
    ...
    const tube = (dx, rot) => {
      g.save(); g.translate(dx, 0.5); g.rotate(rot); g.translate(-dx, -0.5);
      fill(() => roundRectPath(g, dx - 0.11, 0.08, 0.22, 0.84, 0.11));
      g.restore();
    };
    tube(0.32, -0.18);
    tube(0.68, 0.18);
  } else if (sym.id === 'bulb') {
    fill(() => g.arc(0.5, 0.40, 0.32, 0, Math.PI * 2));
    fill(() => roundRectPath(g, 0.36, 0.66, 0.28, 0.14, 0.05));
    fill(() => roundRectPath(g, 0.39, 0.83, 0.22, 0.11, 0.055));
  } else if (sym.id === 'microscope') {
    fill(() => roundRectPath(g, 0.10, 0.84, 0.80, 0.12, 0.06));
    stroke(0.13, () => { g.moveTo(0.62, 0.18); g.lineTo(0.34, 0.62); g.lineTo(0.66, 0.62); });
    fill(() => roundRectPath(g, 0.54, 0.06, 0.22, 0.16, 0.06));
    fill(() => roundRectPath(g, 0.30, 0.68, 0.44, 0.10, 0.05));
...
function renderNoteCover(g, t, L, image, paper, grain) {
  ...
  if (image) {
    const top = y + 86;
    drawContain(g, image, { x: W * 0.16, y: top, w: W * 0.68, h: H - top - PAD - 60 });
  }
}
