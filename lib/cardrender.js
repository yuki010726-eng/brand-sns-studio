 ...
 */
import { getConcept, accentOf, getMark, getCardTheme, getNoteSymbol, getNotePaper, getNoteGrain, noteLayout } from './concepts.js';
 ...
 */
const FONT = (weight, size, family = "'Noto Sans KR', sans-serif") => `${weight} ${size}px ${family}`;
 ...
 */
function fit(g, text, { maxW, maxLines, maxH, sizes, weight, lh, slot, family }) {
...
function drawMark(g, markId, { cx, cy, r, color }) {
  ...
  for (const part of parts) {
    g.stroke(typeof part === 'string' ? new Path2D(part) : markIconRoundRect(...part.rect));
  }
  g.restore();
}
...
function renderMagazine(g, t, L, accent, image, layout = {}) {
  ...
  /**
   * 상단 왼쪽 — 계정 이름 · 상단 오른쪽 — {현재 장}/{전체 장} (2026-08-31, 피그마 반영)
   *
   * ⚠️ `eyebrow` 는 예전에 제목 바로 위 알약 배지였다. 값 자체(`copywriter.js` 의
   * `deckFromCore()`)는 그대로 "01 / 06" 페이지 번호라 새 자리로 그대로 옮겨 쓴다 —
   * 형식(패딩·슬래시)을 바꾸는 것은 카드형·노트형과 공유하는 로직이라 범위 밖이다.
   *
   * ⚠️ 계정 이름은 자유 배치가 없을 때 폭 제한 없이 그대로 찍었다가, 최대 길이(20자)
   * 계정명에서 우상단 페이지 번호와 겹치는 것을 실측으로 확인했다(0.95×글자폭 근사).
   * 페이지 번호가 차지할 폭을 먼저 재서 `fit()` 으로 계정 이름이 그 앞에서 줄어들게 한다.
   */
  const eyebrowStyle = styleOf('eyebrow', 700);
  let eyebrowReserve = 0;
  if (t.eyebrow && !layout.eyebrow) {
    g.font = FONT(eyebrowStyle.weight, eyebrowStyle.fontSize || 30 * eyebrowStyle.scale);
    eyebrowReserve = g.measureText(t.eyebrow).width + 40;
  }
  ...
  if (t.brand) {
    ...
    if (layout.brand) {
      ...
      draw(g, block, { x: box.x, y: box.y - centerLift(block.size, block.lh), color: C.white, align: 'left', boxW: box.w });
      noteBox('brand', box);
    } else {
      const block = fit(g, t.brand, { maxW: Math.max(80, MAXW - eyebrowReserve), maxLines: 1, sizes: scaled([36, 32, 28, 24, 20], brandStyle), weight: brandStyle.weight, lh: 1.2 });
      const box = { x: PAD, y: PAD + 34 - 36 * 0.86, w: MAXW - eyebrowReserve, h: 36 * 1.2 };
      draw(g, block, { x: box.x, y: box.y - centerLift(block.size, block.lh), color: C.white, align: 'left', boxW: box.w });
      noteBox('brand', box);
    }
  }
  ...
  if (t.eyebrow) {
    g.font = FONT(eyebrowStyle.weight, eyebrowStyle.fontSize || 30 * eyebrowStyle.scale);
    g.textBaseline = 'alphabetic';
    const naturalW = g.measureText(t.eyebrow).width;
    const naturalBox = { x: W - PAD - naturalW, y: PAD + 34 - 30 * 0.86, w: naturalW, h: 30 * 1.2 };
    if (layout.eyebrow) {
      const box = denorm(layout.eyebrow);
      const block = fit(g, t.eyebrow, { maxW: box.w, maxH: box.h, maxLines: 1, sizes: scaled([30, 27, 24, 21, 18, 16], eyebrowStyle), weight: eyebrowStyle.weight, lh: 1.2 });
      draw(g, block, { x: box.x, y: box.y - centerLift(block.size, block.lh), color: C.white, align: 'right', boxW: box.w });
      noteBox('eyebrow', box);
    } else {
      g.fillStyle = C.white;
      g.fillText(t.eyebrow, naturalBox.x, PAD + 34);
      noteBox('eyebrow', naturalBox);
    }
  }
...
function renderCardCover(g, t, L, image, markId, theme, layout = {}) {
  ...
  const title = fit(g, t.title, { maxW: MAXW, maxLines: 4, sizes: [80, 72, 64, 56], weight: 900, lh: 1.3, slot: 'title', family: CARD_TITLE_FAMILY });
  ...
  const placedTitle = fit(g, t.title, { maxW: titleBox.w, maxH: titleBox.h, maxLines: 4, sizes: titleStyle.sizes, weight: titleStyle.fontWeight, lh: 1.3, slot: 'title', family: CARD_TITLE_FAMILY });
...
function renderCardPage(g, t, L, image, markId, theme, layout = {}) {
  ...
  const title = t.title ? fit(g, t.title, {
  ...
  const fixedH = TOP_PAD + (title ? title.height + GAP_TITLE_BODY : 0) + GAP_BODY_FOOT + footRowH + BOTTOM_PAD;
  const body = blocks ? fitRich(g, blocks, {
    maxW: savedBodyBox ? Math.max(20, savedBodyBox.w) : MAXW,
    maxH: savedBodyBox ? Math.max(20, savedBodyBox.h) : Math.max(20, maxPanelH - fixedH),
  ...
  if (title) {
    const titleBox = savedHeadBox || { x: PAD, y: cursorY, w: MAXW, h: title.height };
    draw(g, title, { x: titleBox.x, y: titleBox.y - centerLift(title.size, title.lh), color: theme.hex });
  ...
  if (body) {
    ...
    drawRich(g, body, {
      x: bodyTarget.x, y: bodyTarget.y - lift, maxW: bodyTarget.w,
  ...
  if (t.footer) {
    ...
    const natural = { x: PAD, y: footRowY, w: g.measureText(t.footer).width, h: footRowH };
    const target = layout.footer ? denorm(layout.footer) : natural;
    const block = fit(g, t.footer, { maxW: target.w, maxH: target.h, maxLines: 2, sizes: [footStyle.fontSize || 26], weight: footStyle.fontWeight, lh: 1.2, slot: 'footer' });
    draw(g, block, { x: target.x, y: target.y - centerLift(block.size, block.lh), color: L.bodyColor });
    noteBox('footer', target);
  }
  if (t.source) {
    g.font = FONT(500, 22);
...
function renderCardOutro(g, t, L, markId, theme, layout = {}) {
  ...
  const savedBodyBox = layout.body ? denorm(layout.body) : null;
  const body = fit(g, t.body, { maxW: savedBodyBox?.w || MAXW, maxH: savedBodyBox?.h, maxLines: 9, sizes: bodyStyle.sizes, weight: bodyStyle.fontWeight, lh: 1.62, slot: 'body' });
  ...
  if (t.footer) {
    ...
    const target = layout.footer ? denorm(layout.footer) : natural;
    const block = fit(g, t.footer, { maxW: target.w, maxH: target.h, maxLines: 2, sizes: [footerStyle.fontSize || 30], weight: footerStyle.fontWeight, lh: 1.2, slot: 'footer' });
...
function noteTopIcon(g, symbolId, L, { top, size }) {
  drawNoteSymbol(g, symbolId, { x: (W - size) / 2, y: top, size, color: L.titleColor });
}
 ...
 */
function noteCaption(g, text, L, { y, maxW = FX(436), layoutBox, weight = 500 }) {
  const padX = FX(20);
  const padY = FX(15);
  const radius = FX(10);
  ...
  const innerW = block.lines.reduce((m, l) => Math.max(m, g.measureText(l).width), 0);
  const boxW = layoutBox?.w || Math.min(innerW + padX * 2, maxW);
  const boxH = layoutBox?.h || (block.height + padY * 2);
  const boxX = layoutBox ? layoutBox.x : (W - boxW) / 2;
  const boxY = layoutBox ? layoutBox.y : y;

  g.fillStyle = L.accent;
  roundRect(g, boxX, boxY, boxW, boxH, radius);
  g.fill();
  draw(g, block, { x: boxX, y: boxY + padY - centerLift(block.size, block.lh), color: C.white, align: 'center', boxW });
...
function renderNoteCover(g, t, L, image, paper, grain, layout = {}) {
  ...
  const title = fit(g, t.title, {
    maxW: savedTitleBox?.w || MAXW, maxH: savedTitleBox?.h, maxLines: 3,
    sizes: titleStyle.sizes, weight: titleStyle.fontWeight, lh: 1.3, slot: 'title', family: NOTE_TITLE_FAMILY,
  });
  const titleY = FX(70);
  ...
  if (t.body) {
    ...
    sub = fit(g, t.body, { maxW: savedSubBox?.w || MAXW - 80, maxH: savedSubBox?.h, maxLines: 2, sizes: subStyle.sizes, weight: subStyle.fontWeight, lh: 1.3, slot: 'body' });
    const subY = y + FX(22);
    subTarget = savedSubBox || { x: PAD + 40, y: subY, w: MAXW - 80, h: sub.height };
    y = subY + sub.height;
  }
  ...
  if (image) {
    ...
    const naturalBox = { x: (W - size) / 2, y: top, w: size, h: Math.min(size, H - top - PAD) };
...
function renderNoteBody(g, t, L, image, symbolId, paper, grain, layout = {}) {
  ...
  if (t.title) {
    ...
    const title = fit(g, t.title, {
      maxW: saved?.w || MAXW, maxH: saved?.h, maxLines: 2,
      sizes: style.sizes, weight: style.fontWeight, lh: 1.3, slot: 'title', family: NOTE_TITLE_FAMILY,
    });
    const target = saved || { x: PAD, y: FX(126), w: MAXW, h: title.height };
    draw(g, title, { x: target.x, y: target.y - centerLift(title.size, title.lh), color: L.titleColor, align: 'center', boxW: target.w });
  ...
  if (t.body) {
    ...
    const box = noteCaption(g, t.body, L, { y: Math.max(captionTop, FX(489)), layoutBox: saved });
    noteBox('body', box);
    captionTop = box.y + box.h;
  }

  if (t.footer) {
    const style = textStyle(layout, 'footer', 500);
    const saved = layout.footer ? denorm(layout.footer) : null;
    const block = fit(g, t.footer, { maxW: saved?.w || MAXW, maxH: saved?.h, maxLines: 1, sizes: [style.fontSize || 30, 26, 22], weight: style.fontWeight, lh: 1.3, slot: 'footer' });
    const target = saved || { x: PAD, y: Math.max(captionTop + FX(30), H - FX(61) - block.height), w: MAXW, h: block.height };
    draw(g, block, { x: target.x, y: target.y - centerLift(block.size, block.lh), color: L.subColor, align: 'center', boxW: target.w });
    noteBox('footer', target);
  }
}
...
function renderNoteFollow(g, t, L, symbolId, paper, grain, layout = {}) {
  ...
  if (t.body) {
    ...
    const block = fit(g, t.body, {
      maxW: saved?.w || MAXW, maxH: saved?.h, maxLines: 6,
      sizes: style.sizes, weight: style.fontWeight, lh: 1.55, slot: 'body', family: NOTE_TITLE_FAMILY,
    });
    const target = saved || { x: PAD, y: FX(238), w: MAXW, h: block.height };
    draw(g, block, { x: target.x, y: target.y - centerLift(block.size, block.lh), color: L.titleColor, align: 'center', boxW: target.w });
    noteBox('body', target);
  ...
  if (t.footer) {
    ...
    const style = textStyle(layout, 'footer', 500);
    const block = fit(g, t.footer, { maxW: saved?.w || MAXW, maxH: saved?.h, maxLines: 1, sizes: [style.fontSize || 30, 26, 22], weight: style.fontWeight, lh: 1.3, slot: 'footer' });
    const target = saved || { x: PAD, y: bottom + FX(37), w: MAXW, h: block.height };
    draw(g, block, { x: target.x, y: target.y - centerLift(block.size, block.lh), color: L.subColor, align: 'center', boxW: target.w });
    noteBox('footer', target);
  }
}
 ...
 */
export function renderCard(canvas, texts, { conceptId = 'magazine', kind = 'body', image = null, accent, mark, cardTheme, noteSymbol, notePaper, noteGrain, noteInk, layout = {}, extraTexts = [] } = {}) {
  canvas.width = W;
  canvas.height = H;
  ...
  if (concept.id === 'note') {
    ...
    if (role === 'cover') renderNoteCover(g, t, NL, image, paper, grain, layout);
    else if (role === 'outro') renderNoteFollow(g, t, NL, noteSymbol, paper, grain, layout);
