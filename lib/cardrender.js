...
function renderMagazine(g, t, L, accent, image, layout = {}) {
  ...
  if (t.eyebrow && !layout.eyebrow) {
    g.font = FONT(
      eyebrowStyle.weight,
      eyebrowStyle.fontSize || 30 * eyebrowStyle.scale,
    );
  ...
  if (t.brand) {
    ...
    if (layout.brand) {
      ...
      const block = fit(g, t.brand, {
        maxW: Math.max(80, MAXW - eyebrowReserve),
        maxLines: 1,
        sizes: scaled([36, 32, 28, 24, 20], brandStyle),
        weight: brandStyle.weight,
        lh: 1.2,
...
function noteCaption(
  ...
  text,
  L,
  { y, maxW = FX(436), layoutBox, weight = 500 },
) {
  const padX = FX(20);
  const padY = FX(15);
  const radius = FX(10);
  ...
  const innerW = block.lines.reduce(
    (m, l) => Math.max(m, g.measureText(l).width),
    0,
  );
...
function renderNoteBody(g, t, L, image, symbolId, paper, grain, layout = {}) {
  ...
  if (t.footer) {
    ...
    const target = saved || {
      x: PAD,
      y: Math.max(captionTop + FX(30), H - FX(61) - block.height),
      w: MAXW,
      h: block.height,
    };
    draw(g, block, {
