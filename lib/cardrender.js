...
/**
 * 노트형 이미지 한 겹 — 배경이 아니라 **아이콘·일러스트**라 cover 가 아니라 contain 이다.
 *
 * ⚠️ 여기도 `drawInLayout()` 을 쓰면 안 된다 (2026-08-14, 카드형과 같은 이유).
 *    상자를 원본과 다른 비율로 잡는 순간 캔버스 변형이 그림을 눌러 버린다.
 *    contain 은 원래 비율을 지키는 것이 존재 이유인데 그 위에 비균등 scale 을 걸면 의미가 없어진다.
 *    상자를 정하고 **그 상자 안에서 contain 을 다시 계산**한다.
 */
function drawImageIcon(g, image, layout, natural) {
  const target = layout?.image ? denorm(layout.image) : natural;
  drawContain(g, image, target);
...
/**
 * 좌상단 로고 자리 — 실제 로고 이미지가 있으면 그게 우선이고, 심볼은 없을 때 쓰는 대체물이다.
 *
 * ⚠️ 상자를 밖에서 받는다 (2026-08-14). 예전에는 `PAD·LOGO` 로 자리를 스스로 정하고
 *    바깥에서 `drawInLayout()` 이 캔버스 변형으로 옮겼는데, 그러면 상자를 정사각이 아니게
 *    잡는 순간 로고와 심볼이 눌렸다. 여기서 상자를 직접 받아 그리면 비율이 그대로 유지된다.
 *    심볼은 짧은 변에 맞춰 정사각으로 그리고 가운데에 둔다 — 기본 상자(정사각)에서는
 *    여백이 0 이라 예전과 픽셀 단위로 같다.
 */
function noteLogo(g, L, image, symbolId, box) {
  if (image) { drawContain(g, image, box); return; }
 ...
 *
 * 자리 계산(글자 크기·상자)은 예전과 똑같이 먼저 끝내 두고, **실제로 캔버스에 그리는
 * 순서만** 이미지 → 제목 → 부제로 뒤집었다. 예전엔 이미지를 맨 나중에 그려서, 이미지
 * 상자가 자연 위치(글자 아래)에 있을 때는 안 겹쳤지만 사용자가 자유 배치로 상자를
 * 늘리거나 위로 끌어올리면 글자를 그대로 덮어 버렸다. 순서를 뒤집으면 이미지를 어디로
 * 옮기거나 얼마나 키워도 글자가 항상 그 위에 남는다.
 */
function renderNoteCover(g, t, L, image, paper, grain, layout = {}) {
  ...
  if (t.body) {
    ...
    sub = fit(g, t.body, { maxW: savedSubBox?.w || MAXW - 80, maxH: savedSubBox?.h, maxLines: 2, sizes: subStyle.sizes, weight: subStyle.fontWeight, lh: 1.5, slot: 'body' });
    const subY = y + 30;
  ...
  if (image) {
    ...
    drawImageIcon(g, image, layout, natural);
  }
...
function renderNoteBody(g, t, L, image, symbolId, paper, grain, layout = {}) {
  ...
  // 이미지가 없어도 손잡이는 있어야 한다 — 심볼도 옮길 수 있어야 하기 때문이다(예전 동작 유지).
  const logoNatural = { x: PAD, y: PAD, w: LOGO, h: LOGO };
  const logoBox = layout?.image ? denorm(layout.image) : logoNatural;
  noteLogo(g, L, image, symbolId, logoBox);
  noteBox('image', logoBox);
  let y = PAD + LOGO + 44;
...
export function renderCard(canvas, texts, { conceptId = 'magazine', kind = 'body', image = null, accent, mark, cardTheme, noteSymbol, notePaper, noteGrain, noteInk, layout = {}, extraTexts = [] } = {}) {
  ...
  if (concept.id === 'note') {
    ...
    if (roleOf('note', kind) === 'cover') renderNoteCover(g, t, NL, image, paper, grain, layout);
