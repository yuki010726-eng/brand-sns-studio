...
function fitPair(g, a, b, { maxW, maxLines, maxH, sizes, weight, lh, slot }) {
  const cap = (size) => (maxH ? Math.max(1, Math.floor(maxH / (size * lh))) : maxLines);
  for (const size of sizes) {
    g.font = FONT(weight, size);
    const la = a ? wrap(g, a, maxW) : [];
    const lb = b ? wrap(g, b, maxW) : [];
    if (la.length + lb.length <= cap(size)) {
      if (slot) noteSize(slot, size, sizes[0]);
...
function drawExtraTexts(g, items, layout, conceptId) {
  for (const item of items || []) {
    ...
    const fontSize = Math.max(12, Number(saved.fontSize) || Number(item.fontSize) || 44);
    const fontWeight = Number(saved.fontWeight) || Number(item.fontWeight) || 700;
    const block = fit(g, item.text, {
      maxW: box.w, maxH: box.h, maxLines: 8, sizes: [fontSize],
      weight: fontWeight, lh: 1.35, slot: id,
    });
    draw(g, block, { x: box.x, y: box.y, color: item.color || (conceptId === 'note' ? '#191F28' : '#FFFFFF') });
    noteBox(id, box);
  }
}
 ...
 */
function fitRich(g, blocks, { maxW, maxH, sizes, lh, slot }) {
  let last = null;
  for (const size of sizes) {
...
function renderMagazine(g, t, L, accent, image, layout = {}) {
  ...
  if (image) {
    if (layout.image) {
      ...
      drawCover(g, image);
      noteBox('image', { x: 0, y: 0, w: W, h: H });
    }
  } else {
    fallbackBg(g, 'magazine', L);
  }
  ...
  const pair = fitPair(g, t.title, t.highlight, {
    maxW: MAXW, maxLines: 4, sizes: [82, 74, 66, 58], weight: 900, lh: 1.2, slot: ['title', 'highlight'],
  });
...
function renderCardCover(g, t, L, image, markId, theme, layout = {}) {
  ...
  if (image) drawInLayout(g, 'image', { x: 0, y: 0, w: W, h: H }, layout, () => drawCover(g, image));
  ...
  if (t.eyebrow) {
    ...
    drawInLayout(g, 'eyebrow', natural, layout, () => pill(g, t.eyebrow, { x: PAD, y, bg: null, fg: C.white, size: 28, border: C.white, borderW: 2.5 }));
    y += pillH + gapPill;
  }
  drawInLayout(g, 'title', { x: PAD, y, w: MAXW, h: title.height }, layout, () => draw(g, title, { x: PAD, y, color: L.titleColor }));
  if (t.footer) {
    g.font = FONT(700, 32);
    const natural = { x: PAD, y: H - PAD - 32, w: g.measureText(t.footer).width, h: 42 };
    drawInLayout(g, 'footer', natural, layout, () => cardHandle(g, t.footer));
  }
}
...
function renderCardPage(g, t, L, image, markId, theme, layout = {}) {
  ...
  if (t.title) {
    const head = fit(g, t.title, {
      maxW: MAXW - 76, maxLines: 2, sizes: [44, 40, 36, 32], weight: 900, lh: 1.3, slot: 'title',
    });
    ...
    drawInLayout(g, 'eyebrow', { x: PAD, y: headY, w: boxW, h: boxH }, layout, () => {
  ...
  if (t.body) {
    ...
    const body = fitRich(g, blocks, { maxW: MAXW - BP * 2, maxH, sizes: [34, 32, 30, 28, 26, 24], lh: 1.72, slot: 'body' });
    ...
    g.save();
    const bodyTarget = layout.body ? denorm(layout.body) : bodyNatural;
    if (layout.body) {
      g.translate(bodyTarget.x, bodyTarget.y); g.scale(bodyTarget.w / bodyNatural.w, bodyTarget.h / bodyNatural.h); g.translate(-bodyNatural.x, -bodyNatural.y);
    }
    g.fillStyle = L.panel;
  ...
  if (t.footer) {
    g.font = FONT(700, 32);
    drawInLayout(g, 'footer', { x: PAD, y: H - PAD - 32, w: g.measureText(t.footer).width, h: 42 }, layout, () => cardHandle(g, t.footer));
  }
}
...
function renderCardOutro(g, t, L, markId, theme, layout = {}) {
  ...
  const body = fit(g, t.body, { maxW: MAXW, maxLines: 9, sizes: [48, 44, 40, 36], weight: 900, lh: 1.58, slot: 'body' });
  ...
  drawInLayout(g, 'body', bodyNatural, layout, () => draw(g, body, { x: PAD, y, color: C.white, align: 'center', boxW: MAXW }));
  y += body.height;
  ...
  if (t.footer) {
    ...
    const fx = (W - fw) / 2;
    drawInLayout(g, 'footer', { x: fx, y: y + 42, w: fw, h: 42 }, layout, () => g.fillText(t.footer, fx, y + 74));
  }
}
...
function renderNoteCover(g, t, L, image, paper, grain, layout = {}) {
  ...
  const title = fit(g, t.title, { maxW: MAXW, maxLines: 3, sizes: [84, 76, 68, 60], weight: 900, lh: 1.22, slot: 'title' });
  const titleY = PAD + 96;
  drawInLayout(g, 'title', { x: PAD, y: titleY, w: MAXW, h: title.height }, layout, () => draw(g, title, { x: PAD, y: titleY, color: L.titleColor, align: 'center', boxW: MAXW }));
  let y = titleY + title.height;

  if (t.body) {
    const sub = fit(g, t.body, { maxW: MAXW - 80, maxLines: 2, sizes: [34, 31, 28], weight: 400, lh: 1.5, slot: 'body' });
    const subY = y + 30;
    drawInLayout(g, 'body', { x: PAD + 40, y: subY, w: MAXW - 80, h: sub.height }, layout, () => draw(g, sub, { x: PAD + 40, y: subY, color: L.subColor, align: 'center', boxW: MAXW - 80 }));
    y = subY + sub.height;
  }
...
function renderNoteBody(g, t, L, image, symbolId, paper, grain, layout = {}) {
  ...
  if (t.eyebrow) {
    ...
    drawInLayout(g, 'eyebrow', natural, layout, () => noteLabel(g, t.eyebrow, L, y));
    y += 90;
  }

  if (t.title) {
    const title = fit(g, t.title, { maxW: MAXW, maxLines: 3, sizes: [76, 68, 60, 53], weight: 900, lh: 1.22, slot: 'title' });
    const natural = { x: PAD, y, w: MAXW, h: title.height };
    drawInLayout(g, 'title', natural, layout, () => draw(g, title, { x: PAD, y, color: L.titleColor }));
    y += title.height;
    y += 44;
  }
  ...
  if (t.body) {
    const laid = fitRich(g, parseBody(t.body), {
      maxW: MAXW, maxH: H - PAD - y - 40, sizes: [34, 32, 29, 27, 25], lh: 1.68, slot: 'body',
    });
    const natural = { x: PAD, y, w: MAXW, h: laid.height };
    drawInLayout(g, 'body', natural, layout, () => drawRich(g, laid, {
      x: PAD, y, maxW: MAXW,
