 ...
 */
function fit(g, text, { maxW, maxLines, sizes, weight, lh, slot }) {
 ...
 */
function fitPair(g, a, b, { maxW, maxLines, sizes, weight, lh, slot }) {
  for (const size of sizes) {
    g.font = FONT(weight, size);
  ...
  if (slot) { noteClip(slot); noteSize(slot, sizes.at(-1), sizes[0]); }   // 아래에서 줄을 버린다
  const size = sizes.at(-1);
  g.font = FONT(weight, size);
