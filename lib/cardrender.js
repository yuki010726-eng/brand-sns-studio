 ...
 */
let slotSizes = {};
let activeLayout = {}, activeTexts = {}, activeConcept, activeKind;
// CanvasRenderingContext2D는 같은 canvas에서 재사용된다. 편집 중 특정 문구를
// 건너뛰기 위해 fillText를 바꾼 뒤에도, 다음 렌더 시작 시 반드시 원래 함수로
// 되돌려야 한다.
const nativeFillTexts = new WeakMap();
function styleForSlot(slot) {
 const isCardLayout = activeConcept === "card" || activeConcept === "blog";
 const id = isCardLayout && roleOf(activeConcept, activeKind) === "body" && slot === "title" ? "eyebrow" : slot;
 return activeLayout[id] || {};
}

/** 표지 문구는 사용자가 처음 정한 크기에서 시작하되, 두 줄을 지키기 위해 필요하면 줄인다. */
function shrinkToFitTwoLines(sizes, strictLines) {
  if (!strictLines || sizes.length !== 1) return sizes;
  const initial = sizes[0];
  return [1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.25]
    .map((ratio) => Math.max(12, Math.round(initial * ratio)))
    .filter((size, index, values) => values.indexOf(size) === index);
}
function paintText(g, text, x, y, slot, cursor) {
 const ranges = styleForSlot(slot).colorRanges || [];
 const offset = String(activeTexts[slot] || "").indexOf(text, cursor.value);
 cursor.value = (offset < 0 ? cursor.value : offset) + text.length;
 if (!ranges.length || offset < 0) { g.fillText(text, x, y); return; }
 const base = g.fillStyle;
 let index = 0;
 for (const char of text) {
  const range = ranges.findLast(r => offset + index >= r.start && offset + index < r.end);
  g.fillStyle = range?.color || base;
  g.fillText(char, x + g.measureText(text.slice(0, index)).width, y);
  index += char.length;
 }
 g.fillStyle = base;
}
...
const noteSize = (slot, size, biggest) => {
  for (const s of [].concat(slot)) slotSizes[s] = { ...slotSizes[s], size, max: biggest };
};
...
function fit(
  g,
  text,
  { maxW, maxLines, maxH, sizes, weight, lh, slot, family, strictLines },
) {
  const override = styleForSlot([].concat(slot)[0]);
  if (override.fontSize) sizes = [Number(override.fontSize)];
  sizes = shrinkToFitTwoLines(sizes, strictLines);
  if (override.fontWeight) weight = Number(override.fontWeight);
  if (slot) for (const id of [].concat(slot)) slotSizes[id] = { ...slotSizes[id], weight };
  const cap = (size) => {
    const heightCap = maxH ? Math.max(1, Math.floor(maxH / (size * lh))) : maxLines;
    return strictLines ? Math.min(heightCap, strictLines) : heightCap;
  };
  ...
  return { slot, lines, size, height: lines.length * size * lh, weight, lh, family };
}
...
function fitPair(
  ...
  a,
  b,
  { maxW, maxLines, maxH, sizes, weight, lh, slot, tightLines, strictLines },
) {
  sizes = shrinkToFitTwoLines(sizes, strictLines);
  if (slot) for (const id of [].concat(slot)) slotSizes[id] = { ...slotSizes[id], weight };
  const cap = (size) => {
...
function draw(
  ...
  align = styleForSlot(block.slot).textAlign || align;
  boxW = boxW || (styleForSlot(block.slot).w || 0) * W;
    ...
    const lx =
      align === "center"
        ...
        : align === "right"
          ? x + boxW - lineW
          : x;
    paintText(g, line, lx, cur - block.size * (lh - 1) * 0.35, block.slot, cursor);
  }
  return cur;
}
...
function drawPair(g, pair, { x, y, colorA, colorB, align = "left", boxW = 0 }) {
  boxW = boxW || (styleForSlot(pair.slot?.[0]).w || 0) * W;
  g.font = FONT(pair.weight, pair.size);
  g.textBaseline = "alphabetic";
  let cur = y;
  const put = (lines, color, slot) => {
    const cursor = { value: 0 };
    const alignment = styleForSlot(slot).textAlign || styleForSlot(pair.slot?.[0]).textAlign || align;
    g.fillStyle = color;
      ...
      const lx =
        alignment === "center"
          ...
          : alignment === "right"
            ? x + boxW - lineW
            : x;
      paintText(g, line, lx, cur - pair.size * (pair.lh - 1) * 0.35, slot, cursor);
    }
  };
  put(pair.la, colorA, pair.slot?.[0]);
  put(pair.lb, colorB, pair.slot?.[1]);
  return cur;
}
...
function fitRich(
  ...
  const override = styleForSlot(slot);
  if (override.fontSize) sizes = [Number(override.fontSize)];
  if (override.fontWeight) weight = Number(override.fontWeight);
  let last = null;
  for (const size of sizes) {
    ...
    last.slot = slot;
    if (slot) slotSizes[slot] = { ...slotSizes[slot], weight };
    if (last.height <= maxH) {
      if (slot) noteSize(slot, size, sizes[0]);
      return last;
    }
...
function drawRich(
  ...
  const { size, lh } = laid;
  weight = Number(styleForSlot(laid.slot).fontWeight) || weight;
  strongWeight = Math.max(weight, strongWeight);
  const cursor = { value: 0 };
  const align = styleForSlot(laid.slot).textAlign || "left";
  let cur = y;
  ...
  for (const row of laid.rows) {
    ...
    if (row.type === "bar") {
      ...
      for (const line of row.lines) {
        ...
        for (const p of line) {
          ...
          g.fillStyle = boxFg;
          paintText(g, p.text, cx, ty - size * 0.34 * 0.35, laid.slot, cursor);
    ...
    for (const line of row.lines) {
      cur += size * lh;
      const width = lineWidth(g, line, size, weight, strongWeight);
      const space = maxW - row.indent - width;
      let cx = x + row.indent + (align === "center" ? space / 2 : align === "right" ? space : 0);
      for (const p of line) {
        g.font = FONT(p.strong ? strongWeight : weight, size);
        g.fillStyle = p.strong ? strongColor : color;
        paintText(g, p.text, cx, cur - size * (lh - 1) * 0.35, laid.slot, cursor);
        cx += g.measureText(p.text).width;
      }
    }
...
function fallbackBg(g, conceptId, L, theme, paper, grain) {
  ...
  if (conceptId === "card" || conceptId === "blog") {
...
function renderMagazineT1(g, t, L, accent, layout, styleOf, scaled) {
  if (t.brand) {
    ...
    if (layout.brand) {
      ...
      const block = fit(g, t.brand, { slot: "brand",
        maxW: box.w,
        maxH: box.h,
        maxLines: 2,
  ...
  const pair = fitPair(g, t.title, t.highlight, {
    ...
    maxLines: 2,
    strictLines: 2,
    sizes: [82, 74, 66, 58, 50, 44, 38, 32, 28, 24, 20],
    weight: 900,
    lh: 1.2,
  ...
  if (layout.title) {
    ...
    const opair = fitPair(g, t.title, t.highlight, {
      ...
      maxLines: 2,
      strictLines: 2,
      sizes: scaled([82, 74, 66, 58, 50, 44, 38, 32, 28, 24, 20], titleStyle),
  ...
  if (t.footer) {
    if (layout.footer) {
      ...
      const block = fit(g, t.footer, { slot: "footer",
        maxW: box.w,
        maxH: box.h,
        maxLines: 2,
...
function renderCardCover(g, t, L, image, markId, theme, layout = {}) {
  ...
  const title = fit(g, t.title, {
    ...
    maxLines: 2,
    strictLines: 2,
    sizes: [80, 72, 64, 56, 50, 44, 38, 32, 28, 24, 20],
    weight: 900,
    lh: 1.3,
    slot: "title",
  });
  ...
  const titleStyle = textSizes(layout, "title", [80, 72, 64, 56, 50, 44, 38, 32, 28, 24, 20], 900);
...
function renderNoteCover(g, t, L, image, paper, grain, layout = {}) {
  ...
  const titleStyle = textSizes(layout, "title", [100, 90, 80, 70, 60, 54, 48, 42, 36, 30, 24, 20], 400);
 ...
 */
/** Draw the blog-only 3:2 PR banner.  This intentionally does not share the
 * vertical card renderer: merely cropping a 4:5 card made the supplied wide
 * references look unrelated and cut off the hero image. */
function renderBlogBanner(canvas, t, { image, kind, extraTexts = [], L }) {
  const g = canvas.getContext("2d");
  const w = BLOG_W;
  const h = BLOG_H;
  const navy = L.brand || "#0C315E";
  const ink = L.bodyColor || "#172B4D";
  const isOutro = roleOf("blog", kind) === "outro";

  g.clearRect(0, 0, w, h);
  const bg = g.createLinearGradient(0, 0, w, h);
  bg.addColorStop(0, isOutro ? "#0B315F" : "#F7FBFF");
  bg.addColorStop(1, isOutro ? "#123F75" : "#DCEAF8");
  g.fillStyle = bg;
  g.fillRect(0, 0, w, h);

  // The generated source occupies the right side, just like the plaque,
  // newspaper and laptop in the supplied references.  Crop it to fill rather
  // than stretching it, so a 3:2 prompt stays visually faithful.
  if (image && !isOutro) {
    const target = { x: 590, y: 0, w: 610, h: 800 };
    ...
    g.drawImage(image, sx, sy, sw, sh, target.x, target.y, target.w, target.h);
    const veil = g.createLinearGradient(545, 0, 780, 0);
    veil.addColorStop(0, "#F7FBFF");
    veil.addColorStop(1, "rgba(247,251,255,0)");
    g.fillStyle = veil;
    g.fillRect(520, 0, 310, h);
  }

  const write = (text, x, y, maxWidth, size, weight, color, line = 1.22) => {
    const words = String(text || "").trim().split(/\s+/).filter(Boolean);
    if (!words.length) return y;
    g.font = `${weight} ${size}px "Noto Sans KR", sans-serif`;
    g.fillStyle = color;
    let row = "";
    let yy = y;
  ...
  const light = isOutro ? "#FFFFFF" : ink;
  const sub = isOutro ? "rgba(255,255,255,.85)" : navy;
  let y = 72;
  y = write(t.eyebrow || "대한민국 고객만족 신뢰도 대상", 62, y, 500, 23, 600, sub, 1.2);
  y += 30;
  y = write(t.title || t.eyebrow || "브랜드 소식", 62, y, 520, isOutro ? 62 : 58, 900, light, 1.18);

  if (!isOutro) {
    const body = t.body || "핵심 정보를 쉽고 신뢰감 있게 전달합니다.";
    const panelY = Math.min(525, y + 36);
    g.fillStyle = "rgba(255,255,255,.90)";
    g.beginPath();
    g.roundRect(54, panelY, 500, 155, 22);
    g.fill();
    write(body, 82, panelY + 48, 440, 23, 500, ink, 1.5);
  } else {
    write(t.body || "자세한 내용은 본문에서 확인해 보세요.", 64, Math.min(h - 175, y + 42), 600, 27, 500, "rgba(255,255,255,.92)", 1.45);
  }

  g.font = '700 24px "Noto Sans KR", sans-serif';
  g.fillStyle = isOutro ? "#DCEAF8" : navy;
  g.fillText(t.footer || "KCST", 62, h - 58);
  // The visual editor needs real hit boxes even though this layout is bespoke.
  boxes = {
    eyebrow: { x: 62, y: 42, w: 500, h: 62 },
    title: { x: 62, y: 110, w: 520, h: 250 },
    body: { x: 54, y: 385, w: 500, h: 295 },
    footer: { x: 62, y: h - 92, w: 300, h: 55 },
  };
  extraTexts.forEach((item) => {
    if (!item?.text) return;
    const x = (item.x ?? 0.5) * w;
    const y = (item.y ?? 0.5) * h;
    write(item.text, x, y, Math.max(120, (item.w ?? 0.25) * w), item.fontSize || 24, item.fontWeight || 500, item.color || light);
  });
}
...
export function renderCard(
  ...
  const isBlog = conceptId === "blog";
  canvas.width = isBlog ? BLOG_W : W;
  canvas.height = isBlog ? BLOG_H : H;
  const g = canvas.getContext("2d");
  let nativeFillText = nativeFillTexts.get(g);
  if (!nativeFillText) {
    nativeFillText = g.fillText.bind(g);
    nativeFillTexts.set(g, nativeFillText);
  }
  g.fillText = nativeFillText;
  // In the card editor, glyphs are rendered by the DOM layer. Keep canvas
  // measurements/layout intact while leaving its background artwork in place.
  if (suppressText) {
    g.fillText = () => {};
  } else if (Array.isArray(suppressTextValues) && suppressTextValues.length) {
    // 캔버스 위 DOM 편집기가 맡은 문구만 빼고 나머지(매거진의 강조 문구 등)는
    // 그대로 그린다. 제목+강조 문구를 한 번에 숨기면 편집 중 강조색이 사라진다.
    const values = suppressTextValues.map((value) => String(value || "")).filter(Boolean);
    g.fillText = (text, ...args) => {
      const rendered = String(text || "");
      if (rendered && values.some((value) => value.includes(rendered))) return;
      nativeFillText(text, ...args);
    };
  }
  ...
  activeKind = kind;
  activeTexts = { ...texts, ...Object.fromEntries(extraTexts.map(item => [`extra-${item.id}`, item.text])) };
  slotSizes = {};
  boxes = {};
  lines = {};
  ...
  const theme =
    concept.id === "blog"
      ? { hex: L.brand || "#0C315E", light: "#DCEAF8" }
      : getCardTheme(cardTheme);

  if (concept.id === "blog") {
    renderBlogBanner(canvas, texts || {}, { image, kind, layout, extraTexts, theme, L });
    return;
  }
  ...
  if (concept.id === "card") {
    const role = roleOf(concept.id, kind);
