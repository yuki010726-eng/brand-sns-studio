...
/**
 * 챗봇 대화방은 생성 조건뿐 아니라 채널과 그 채널의 시안별로 분리한다.
 * 각 값을 인코딩해 구분자가 포함된 사용자 입력도 다른 대화방과 충돌하지 않게 한다.
 */
export function copyChatContextKey(generationKey, channelId, variantIndex) {
  const generation = String(generationKey || '').trim();
  const channel = String(channelId || '').trim();
  const variant = Number(variantIndex);
  if (!generation || !channel || !Number.isInteger(variant) || variant < 0) return '';

  return `copy-chat:v2:g=${encodeURIComponent(generation)}:c=${encodeURIComponent(channel)}:v=${variant}`;
}
...
/** 새 요약을 저장한다. 비동기 작업 중 계정이 바뀌면 이전 결과는 저장하지 않는다. */
export async function saveMemorySummary(summary, messageCount, expectedUserId) {
  const user = getUser();
  if (!user) return { ok: false, skipped: true };
  if (expectedUserId && user.id !== expectedUserId) {
    return { ok: false, skipped: true, reason: 'user_changed' };
  }
  const sb = await getClient();
  if (!sb) return { ok: false, skipped: true };
  // getClient()를 기다리는 사이에도 로그아웃/계정 전환이 일어날 수 있다.
  if (getUser()?.id !== user.id) {
    return { ok: false, skipped: true, reason: 'user_changed' };
  }
  const { error } = await sb.from('user_writing_memory').upsert({
    user_id: expectedUserId || user.id,
    summary,
