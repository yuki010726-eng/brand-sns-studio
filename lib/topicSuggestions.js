...
import { cacheKey, clearSuggestion, readSuggestion, writeSuggestion } from "./suggestionCache.js";
...
export function topicSuggestionKey(product, previous = []) {
  const proofs = [...new Set((product?.voice?.proof || []).map(clean).filter(Boolean))];
  return cacheKey("topic-v1", JSON.stringify({
    productId: String(product?.id || "").trim(),
    productName: String(product?.name || "").trim(),
    proofs,
    previous: previous.map(clean).filter(Boolean),
  }));
}

export async function getTopicSuggestions(product, previous = [], options = {}) {
  const contextFacts = String(options.facts || '').split(/\n+/).map(clean).filter(Boolean);
  const proofs = [...new Set([...contextFacts, ...(product?.voice?.proof || []).map(clean)].filter(Boolean))];
  if (!proofs.length) return fallbackTopics(product, previous);

  const key = topicSuggestionKey(product, previous);
  if (options.forceRefresh) clearSuggestion(key);
  else {
    const cached = readSuggestion(key);
    if (cached?.length >= TOPIC_COUNT) return cached.slice(0, TOPIC_COUNT);
  }
...
${materials.map((item) => `- ${item}`).join("\n")}`;
  ...
  try {
    ...
    if (generated.length >= TOPIC_COUNT) {
      return writeSuggestion(key, generated.slice(0, TOPIC_COUNT));
    }
  } catch (error) {
