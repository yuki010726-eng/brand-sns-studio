...
export function objectsFor(conceptId, kind) {
  ...
  if (conceptId === 'note') return [
    ...(role === 'body' ? [{ id: 'eyebrow', type: 'text', label: '라벨' }] : []),
    { id: 'title', type: 'text', label: '제목' },
    { id: 'body', type: 'text', label: role === 'cover' ? '부제' : '본문' },
    { id: 'image', type: 'image', label: role === 'cover' ? '일러스트' : '아이콘' },
  ];
  return [];
}
...
export function defaultsFor(conceptId, card, product) {
  ...
  if (conceptId === 'magazine') {
    ...
    // 두 줄 합쳐 상한 안에 들어오게 먼저 줄인다 — 그래야 splitHeadline 이 "…" 없이 나눌 수 있다
    const [head, tail] = card.kind === 'outro'
      ? [card.title, '']
      : splitHeadline(headline(card.title, MAG_LINE_MAX * 2));
    return pick({
      brand: product.short,
