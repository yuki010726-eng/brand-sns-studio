...
export function defaultsFor(conceptId, card, product) {
  ...
  if (conceptId === "card") {
    ...
    if (role === "outro") {
      return pick({
        // 생성된 마지막 카드의 결론을 버리지 않는다. outro 레이아웃에는 title 슬롯이
        // 없으므로 제목과 본문을 한 상자에 합쳐야 두 문구가 모두 이미지에 보인다.
        body: [card.title, card.body].filter(Boolean).join("\n\n"),
        footer: product.handle,
      });
    }
    return pick({
  ...
  if (role === "outro") {
    return pick({
      // 노트형 마무리도 생성 결과를 그대로 사용한다. 고정 팔로우 문구로 덮어쓰면
      // 덱의 마지막 핵심이 사라진다.
      body: [card.title, card.body].filter(Boolean).join("\n\n"),
      footer: product.handle,
    });
