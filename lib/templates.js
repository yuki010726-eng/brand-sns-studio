...
export function roleOf(conceptId, kind) {
  ...
  if (kind === "follow")
    return conceptId === "card" || conceptId === "blog" || conceptId === "note" ? "outro" : "body";
  return "body";
}
...
// 블로그형은 카드형의 검증된 텍스트 슬롯을 공유한다. 색과 이미지 프롬프트는
// lib/concepts.js 의 블로그형 정의를 사용한다.
SLOTS.blog = SLOTS.card;
...
/**
 * 자유 배치 오브젝트 id → 실제 문구가 담긴 텍스트 필드 id.
 *
 * 대부분은 같은 이름이다(오브젝트 "title" ↔ 텍스트 필드 `texts.title`). 딱 하나
 * 예외가 있다 — **카드형 본문(role='body')** 은 렌더러(`lib/cardrender.js`
 * `renderCardPage`)가 흰 패널 맨 위 테마색 소제목을 `texts.title` 에서 읽으면서도
 * 자유 배치 상자 이름은 표지와 맞추려고 `"eyebrow"` 로 등록한다(`noteBox("eyebrow", …)`).
 * 미리보기 위에서 더블클릭으로 글을 고칠 때(`CanvasPreview.jsx`) 이 상자를 눌렀다고
 * `texts.eyebrow` 를 고치면 안 된다 — 화면에 보이는 그 글자는 `texts.title` 이다.
 *
 * ⚠️ 새 오브젝트를 추가하거나 카드형 본문 배치를 바꿀 때는 `renderCardPage()` 가
 *    실제로 어느 `t.<field>` 를 그 상자에 그리는지 다시 확인하고 여기도 맞출 것.
 */
export function slotIdForObject(conceptId, kind, objId) {
  if (conceptId === "card" && roleOf(conceptId, kind) === "body" && objId === "eyebrow") {
    return "title";
  }
  return objId;
}
