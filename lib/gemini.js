...
/** 내장 키(`config.local.js`)가 있으면 그것이 이긴다 — 이유는 lib/openai.js 의 getKey 주석 참고 */
export const getKey = () => builtInKey('gemini') || localStorage.getItem(KEY_STORE) || '';
export const hasKey = () => getKey().trim().length > 0;
export const isBuiltInKey = () => hasBuiltInKey('gemini');
...
/** 고정 값이 이 목록에 있을 때만 이긴다 — 이유는 openai.js 의 getTextModel 주석 참고 */
export const getTextModel = () => {
  const pinned = pinnedTextModel();
  if (pinned && TEXT_MODELS.some((m) => m.id === pinned)) return pinned;
  return localStorage.getItem(TEXT_MODEL_STORE) || TEXT_MODELS[0].id;
};
...
export async function generateText(prompt, opts = {}) {
  ...
  const json = await res.json();
  // 제공자마다 이름이 달라 있는 쪽을 그대로 넘긴다 (openai.js 의 onUsage 와 같은 용도)
