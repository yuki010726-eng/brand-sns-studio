 ...
 *
 * ⚠️ **글이 길어지라고 바꾼 것이 아니다** (요청자: "지금 글 양도 나쁘지 않아").
 *    아래 값은 지금 실제로 나오는 본문 길이에 맞춰 잡았다 — 화면에 보이는 총 길이는 그대로다.
 *    바뀐 것은 **숫자가 무엇을 가리키는지**뿐이고, 그래야 장수를 바꿔도 본문이 흔들리지 않는다.
 */
const BLOG_PLAN = {
  1: { points: 2, min: 600, max: 850 },
  2: { points: 2, min: 620, max: 880 },
  3: { points: 2, min: 650, max: 900 },
  4: { points: 3, min: 700, max: 950 },
  5: { points: 3, min: 750, max: 1000 },
  6: { points: 3, min: 800, max: 1100 },
};
