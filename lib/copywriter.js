...
const imageSlot = (n, caption) =>
  `📷 [이미지 ${n} · ${CARD_ROLE[n - 1]}] 카드뉴스 ${n}번을 여기에 넣으세요`
  + (caption ? `\n⤷ ${caption}` : '');
 ...
 */
export function buildCore({ product: p, topic, tone, variant = 0 }) {
  const v = p.voice;
...
function blogCopy(core) {
  ...
  const v = p.voice;
  const deck = deckFromCore(core);   // 캡션을 카드에서 가져온다 — 인스타 카드와 같은 문구가 된다
  ...
  const sections = points.map(({ q, a }, i) => {
    ...
    return `## ${q}\n\n${breathe(a + (extra ? ` ${extra}` : ''))}\n\n${imageSlot(i + 2, captionOf(deck[i + 1]))}`;
  }).join('\n\n');
...
/** 코어에서 카드 6장을 만든다. 블로그 캡션도 여기서 나온다. */
function deckFromCore(core) {
...
export function buildDeck(ctx) {
  return deckFromCore(buildCore({ ...ctx, variant: ctx.variant ?? 0 }));
}
