 ...
 *
 * ⚠️ 태그 **문자열 자체는 만들지 않는다.** `data/products.js` 에 승인된 것만 쓴다
...
function tagsFor(tags, seed, n) {
  ...
  const start = ((seed % src.length) + src.length) % src.length;
  return Array.from({ length: take }, (_, i) => `#${src[(start + i) % src.length]}`).join(' ');
}
...
/**
 * 소제목 표시 — 마크다운 `##` 을 쓰지 않는다.
 *
 * ⚠️ 붙여넣을 곳(네이버 블로그·인스타그램·쓰레드)이 전부 순수 텍스트라 샵과 별표가 글자로 찍힌다.
 *    AI 경로(`lib/copyai.js` 의 `stripMarkdown()`)도 같은 표시로 바꾼다 —
...
/* ------------------------------------------------------------
   분량 계획 — 장수에 따라 **글도 같이 줄어든다** (2026-08-13)

   ⚠️ 예전 규칙은 "카드가 몇 장이든 글은 그대로"였다. 그래서 1장을 골라도
      블로그가 1,800~2,400자로 나왔다. 요청자 지적:
      "내용·장수가 적어지면 그에 맞게 줄어들고, 한 섹션 내용이 너무 많아 안 읽힌다."

   그래서 두 가지를 함께 조인다.
     ① 소제목(=핵심) 개수를 장수에 맞춘다
     ② 목표 분량 자체를 낮춘다 (6장 기준 1,800~2,400 → 1,200~1,600)
   ...
   ------------------------------------------------------------ */
const BLOG_PLAN = {
  1: { points: 2, min: 700, max: 1000 },
  2: { points: 2, min: 800, max: 1100 },
  3: { points: 2, min: 900, max: 1200 },
  4: { points: 3, min: 1000, max: 1300 },
  5: { points: 3, min: 1100, max: 1450 },
  6: { points: 3, min: 1200, max: 1600 },
};

/** 장수별 분량 계획. 프롬프트(copyai.js)와 규칙 기반이 **같은 표**를 본다. */
export const blogPlanFor = (size) => BLOG_PLAN[clampDeckSize(size)];

/** 세 채널이 공통으로 다루는 핵심 개수 — 장수를 따라간다. 카드뉴스 본문 장수와 맞물린다. */
...
${imageSlot(i + 1, labels[i], captionOf(deck[i]))}`;
   ...
   */
  const lead = breathe(`${pick(BLOG_LEADS, TONE_HOOK[core.tone] + core.variant)(topic)} 필요한 것만 골라 봤습니다.`);
   ...
   */
  /**
   * ⚠️ **근거 문장을 덧대지 않는다** (2026-08-13). 예전에는 문단마다 `proof` 를 한 줄씩
   *    더 붙여 살을 불렸는데, 요청자 지적이 정확히 그 지점이었다 —
   *    "한 가지 섹션에 대한 내용이 너무 많아. 글이 너무 많으니깐 잘 안 읽혀."
   *    한 문단은 **한 가지만** 말한다. 근거는 이미 요약(> 줄)에 들어가 있다.
   */
  const sections = points
    .map(({ q, a }, i) => `${HEAD_MARK} ${q}\n\n${breathe(a)}${slotAt('body', i)}`)
    .join('\n\n');
  ...
  return [
    ...
    sections,
    '',
    `${HEAD_MARK} ${core.objectionHead || '그래도 망설여진다면'}`,
    '',
    breathe(objection),
    slotAt('note').trim() ? `
${slotAt('note').trim()}` : '',
    ...
    closing,
    '',
    // ⚠️ '비용'을 쓰지 않는다 (2026-08-13). 비용은 상담 이후 영역이라 콘텐츠에서 꺼내지 않는다.
...
${slotAt('outro').trim()}` : '',
    ...
    tagsFor(p.hashtags, TONE_HOOK[core.tone] + core.variant, 3),
...
export function buildDeck(ctx) {
   ...
   */
  if (!ctx.core?.fromAI) throw new Error('AI로 생성한 주제 뼈대가 필요합니다.');
  return deckFromCore(ctx.core, ctx.cardCount);
}
