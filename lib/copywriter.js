 ...
 * - AI 경로의 마무리 문장은 이번 글의 내용에 맞춰 직접 생성한다
...
function byTopic(items, topic, textOf = (x) => x) {
  // Ranking is optional: incomplete product settings should render as an
  // empty source list, not crash the card preview.
 ...
 */
const BLOG_PLAN = {
  1: { points: 3, min: 1300, max: 2600 },
  2: { points: 3, min: 1300, max: 2600 },
  3: { points: 3, min: 1300, max: 2600 },
  // 본문 핵심 소제목 수입니다. 실제 원고에는 마지막 마무리 소제목이 하나 더 붙습니다.
  // 따라서 4/5/6장일 때 최종 소제목은 각각 5~6/6~7/7~8개가 됩니다.
  4: { points: 4, min: 1300, max: 2600 },
  5: { points: 5, min: 1300, max: 2600 },
  6: { points: 6, min: 1300, max: 2600 },
  7: { points: 7, min: 1300, max: 2600 },
};
...
/** 규칙 기반 폴백도 선택한 카드뉴스 장수에 맞는 핵심 개수를 쓴다. */
export const pointsFor = (size) => blogPlanFor(size).points;
...
export function buildCore({ product: p, topic, tone, variant = 0, cardCount = DECK_SIZE }) {
  ...
  const configuredPoints = byTopic(v.qa, topic, (x) => `${x?.q || ''} ${x?.a || ''}`)
    .filter((x) => x && typeof x === 'object' && (x.q || x.a));
  // A product can be saved before its curated Q&A is complete. Use its
  // approved facts as card material so deckFromCore always receives points.
  const fallbackPoints = [
    ...(Array.isArray(p.facts) ? p.facts : []),
    ...(Array.isArray(p.benefits) ? p.benefits : []),
    ...(Array.isArray(v.proof) ? v.proof : []),
    p.summary,
    '상품의 공식 정보를 확인해 주세요.',
  ]
    .map((a) => String(a || '').trim())
    .filter(Boolean)
    .map((a) => ({ q: topic || p.short || p.name || '안내', a }));
  const points = (configuredPoints.length ? configuredPoints : fallbackPoints).slice(0, pointsFor(size));
  ...
  return {
    ...
    hook: pick(Array.isArray(v.hooks) && v.hooks.length ? v.hooks : [topic || p.short || p.name || '안내'], TONE_HOOK[tone] + variant),
    summary,
    points,
    extra,
    // 본문 항목과 겹치는 문장은 덜어낸다 — 같은 말을 두 번 하면 글이 빈약해 보인다
    objection: dropSaid(v.objection, [...said, ...summary]),
    // AI를 쓸 수 없는 폴백에서도 DB의 고정 마무리 문구는 가져오지 않는다.
    closing: `${topic}은 우리 상황에 맞는 조건인지부터 확인해 보세요.`,
    cta: `${topic}의 핵심 조건을 확인해 보세요.`,
    events: openEvents(p),
  };
}
...
function instagramCopy(core) {
  ...
  const blocks = [
    ...
    `${pick(IG_SAVES, seed)}\n${cta}\n${p.handle}`,
    `.\n.\n.\n${tagsFor(p.hashtags, tagSeed(core), 6 + (tagSeed(core) % 5))}`,
  ];
 ...
 *   2장  표지 · 마무리
 *   3장  표지 · 본문1 · 마무리
 *   5장  표지 · 본문1 · 본문2 · 본문3 · 마무리
 *   6장  표지 · 본문1 · 본문2 · 본문3 · 본문4 · 마무리
 *   7장  표지 · 본문1 · 본문2 · 본문3 · 본문4 · 본문5 · 마무리
 */
const DECK_PLAN = {
  1: ['cover'],
  2: ['cover', 'outro'],
  3: ['cover', 'body', 'outro'],
  // 이전에 저장된 4장 선택값도 새 5장 구성으로 보여 준다.
  4: ['cover', 'body', 'body', 'body', 'outro'],
  5: ['cover', 'body', 'body', 'body', 'outro'],
  6: ['cover', 'body', 'body', 'body', 'body', 'outro'],
  7: ['cover', 'body', 'body', 'body', 'body', 'body', 'outro'],
};
...
function deckFromCore(core, size = DECK_SIZE) {
  ...
  // 오래된 빈 outline도 DB 문구로 되돌리지 않고 현재 주제에 맞춰 복구한다.
  const closing = core.closing || `${topic}은 우리 상황에 맞는 조건인지부터 확인해 보세요.`;
  // 마무리 카드 제목도 AI가 이번 글에 맞춰 만든다. 오래된 코어만 주제 기반으로 복구한다.
  const cta = core.cta || `${topic}의 핵심 조건을 확인해 보세요.`;
  /**
...
export function buildDeck(ctx) {
   ...
   */
  // A stored topic core is optional. It may be an older rule-based value,
  // so never require an AI-generated outline to render card copy.
  const core = ctx.core?.points?.length
    ? ctx.core
