...
export function instagramConfig() {
  ...
  const supabaseUrl = (process.env.SUPABASE_URL || SUPABASE.url || '').trim();
...
export function requireInstagramConfig() {
  ...
  console.log('[instagram-debug] redirectUri:', JSON.stringify(config.redirectUri), 'length:', config.redirectUri.length);
  try {
...
export async function exchangeInstagramCode(code, config) {
  console.log(
    '[instagram-debug] exchange attempt',
  ...
  // Never write OAuth access tokens to application logs.
  console.log('[instagram-debug] code exchange response', {
    status: response.status,
    userId: body?.user_id || null,
    permissions: Array.isArray(body?.permissions) ? body.permissions : null,
    error: body?.error_message || body?.error?.message || null,
  });
...
export async function exchangeLongLivedToken(shortToken, config) {
  ...
  const body = await response.json().catch(() => ({}));
  console.log('[instagram-debug] long-lived token response', {
    status: response.status,
    expiresIn: body?.expires_in || null,
    error: body?.error?.message || body?.error_message || null,
  });
...
export async function getInstagramProfile(accessToken) {
  ...
  const response = await fetch(url);
  const body = await response.json().catch(() => ({}));
  console.log('[instagram-debug] profile response', {
    status: response.status,
