...
import { findBanned, buildDeck, captionOf, roleLabels, clampDeckSize, HEAD_MARK } from './copywriter.js';
...
4) 도입 — 이걸 지금 알아야 하는 이유부터. 인사말로 시작하지 않습니다.
5) "${HEAD_MARK} 소제목" 문단 **3개**. 위 「이번 게시물의 핵심」 3가지를 **순서대로 하나씩** 다룹니다.
   상품 전반을 소개하는 문단을 끼워 넣지 마세요. 한 문단이라도 각도에서 벗어나면 실패입니다.
6) "${HEAD_MARK} 그래도 망설여진다면" 문단에서 한계를 솔직히 씁니다
7) "${HEAD_MARK} 정리하면" 문단 — 허용된 마무리 문장을 원문 그대로 한 줄 쓰고, 접수·확인 안내를 덧붙입니다.
