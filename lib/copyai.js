...
import { findBanned, buildCore, buildDeck, captionOf, roleLabels, clampDeckSize } from './copywriter.js';
...
■ 깊이: **한 주제를 끝까지 파고듭니다.**
주제를 벗어나 상품 전반 소개로 흘러가는 것이 가장 흔한 실패입니다.
받은 목차 3가지를 각각 **왜 그런지 · 어떻게 하는지 · 무엇을 조심해야 하는지**까지 풀어 씁니다.
사실 하나를 한 문장으로 던지고 넘어가지 않습니다. 그 사실이 읽는 사람에게 무슨 의미인지까지 씁니다.
...
function channelRules(channelId, p, size = 6) {
  if (channelId === 'blog') {
...
5) "## 소제목" 문단 **3개**. 위 「이번 게시물의 핵심」 3가지를 **순서대로 하나씩** 다룹니다.
   상품 전반을 소개하는 문단을 끼워 넣지 마세요. 한 문단이라도 주제에서 벗어나면 실패입니다.
6) "## 그래도 망설여진다면" 문단에서 한계를 솔직히 씁니다
7) "## 정리하면" 문단 — 허용된 마무리 문장을 원문 그대로 한 줄 쓰고, 접수·확인 안내를 덧붙입니다.
8) 개요표를 이 형식으로 씁니다. 한 줄에 하나씩.
   🔔 상품 · ${p.name}
...
⚠️ 카드가 ${size}장뿐이어도 **글은 기승전결을 전부 갖춥니다.** 소제목 3개·반론·정리를 빼지 마세요.
...
function coreBlock(ctx) {
  /**
   * ⚠️ `ctx.core` 가 오면 그것을 쓴다 — AI 가 주제로 짠 뼈대(`lib/outline.js`)다.
   *    없으면 규칙 기반으로 떨어진다(키가 없거나 뼈대 생성이 실패한 경우).
   */
  const core = ctx.core || buildCore({ ...ctx, variant: ctx.variant ?? 0 });
  return [
    '■ 이번 게시물에서 다룰 3가지 (세 채널이 공통으로 다룹니다)',
    ...core.points.map((x, i) => `${i + 1}. ${x.q}\n   → ${x.a}`),
    '',
    `한계·반론으로 다룰 것: ${core.objection}`,
    `마무리 문장: ${core.closing}`,
    '',
    '**이 3가지는 목차이지 완성된 글이 아닙니다.**',
    '위 한 줄 설명을 그대로 옮겨 붙이지 말고, 각 항목을 실제로 **풀어서 자세히** 쓰세요.',
    '왜 그런지, 어떻게 하는지, 무엇을 조심해야 하는지까지 짚어 줍니다.',
    '',
...
function buildPrompt(ctx, channelId, retryNote) {
  ...
  return [
    ...
    '── 자료 끝 ──',
    '',
    channelRules(channelId, product, clampDeckSize(ctx.cardCount)),
    '',
...
export function promptKeyOf(channelId, s) {
  const base = `${s.productId}|${String(s.topic || '').trim()}|${s.tone}`;
