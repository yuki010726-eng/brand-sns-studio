...
import { findBanned, buildCore, buildDeck, captionOf } from './copywriter.js';
...
function repairBlog(text, ctx) {
  ...
  // 3) 빠진 이미지 자리는 끝에 채운다 (해시태그·개요표 앞)
  const have = new Set(lines.map((l) => l.match(SLOT_RE)?.[1]).filter(Boolean).map(Number));
  const missing = [1, 2, 3, 4, 5, 6].filter((n) => !have.has(n));
  if (missing.length) {
    let tail = lines.length;
    for (let i = lines.length - 1; i >= 0; i--) {
      if (/^\s*(#\S|🔔)/.test(lines[i])) tail = i;
    }
    const block = missing.flatMap((n) => ['', imageSlotLine(n), `${CAPTION_MARK} ${caption(n)}`]);
