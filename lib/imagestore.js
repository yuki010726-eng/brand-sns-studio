 ...
 */
export const imageKey = (productId, conceptId, index, postId = '', scope = '') => {
  const base = postId
    ? `${postId}-${productId}-${conceptId}-${index}`
    : `${productId}-${conceptId}-${index}`;
  return scope ? `${base}-${scope}` : base;
};

/**
 * Blog banners belong to an individual AI draft ("시안"), not just to the
 * post and template.  Without this suffix, switching from 시안 2 to 시안 3
 * made both previews read and overwrite the same IndexedDB blob.
 */
export const imageScopeForState = (state) => {
  if (state?.concept !== 'blog') return '';
  const active = state?.activeAiRun;
  const run = typeof active === 'object' ? active?.blog : active;
  return Number.isInteger(run) && run >= 0 ? `blog-draft-${run}` : '';
};

export const imageKeyForState = (state, index) =>
  imageKey(
    state?.productId,
    state?.concept,
    index,
...
/**
 * Keep pre-change images visible until a draft gets its own upload. New
 * writes always use the scoped key above, so they no longer affect a sibling
 * draft.
 */
export async function getImageForState(state, index) {
  const scoped = imageKeyForState(state, index);
  const blob = await getImage(scoped);
  if (blob || !imageScopeForState(state)) return blob;
  return getImage(imageKey(state?.productId, state?.concept, index, state?.postId));
}

/** 계정은 하나뿐이라 키도 하나다 — 프로필 화면과 인스타그램 게시글 미리보기가 함께 본다. */
