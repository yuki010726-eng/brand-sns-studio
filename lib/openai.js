...
import { builtInKey, hasBuiltInKey, pinnedTextModel } from './localconfig.js';
 ...
 * 키를 읽는 유일한 곳.
 *
 * ⚠️ **내장 키(`config.local.js`)가 있으면 그것이 이긴다.** 화면에서 넣은 키보다 우선이다.
 *    반대로 두면 예전에 넣어 둔 낡은 키가 남아 있는 브라우저에서 내장 키가 안 먹는다.
 *    내장 키는 `.gitignore` 된 로컬 파일에서만 오고 localStorage 에는 쓰지 않는다.
 */
export const getKey = () => builtInKey('openai') || localStorage.getItem(KEY_STORE) || '';
export const hasKey = () => getKey().trim().length > 0;
/** 내장 키로 도는 중인지 — 화면이 입력칸 대신 안내를 보여줄지 판단한다 */
export const isBuiltInKey = () => hasBuiltInKey('openai');
/** ⚠️ 내장 키는 지우지 않는다. 파일에서 오는 값이라 여기서 손댈 수 없다. */
...
export const getTextModel = () => {
  /**
   * ⚠️ `config.local.js` 의 고정 값이 가장 먼저다 (요청자 지시: OpenAI · Terra 로 고정).
   *    `generateText()` 가 이 함수를 직접 부르므로 **고정은 반드시 여기에 있어야 한다** —
   *    llm.js 에만 두면 화면에는 Terra 로 보이는데 실제 호출은 다른 모델로 나간다.
   *    목록에 없는 값이면 무시한다. 오타 하나로 글쓰기가 죽으면 안 된다.
   */
  const pinned = pinnedTextModel();
  if (pinned && TEXT_MODELS.some((m) => m.id === pinned)) return pinned;
...
/**
 * @param {string} prompt
 * @param {{system?:string, temperature?:number, maxOutputTokens?:number, reasoningEffort?:'low'|'medium'|'high',
 *          signal?:AbortSignal, onUsage?:(usage:object)=>void}} [opts]
 *   `reasoningEffort` 를 주지 않으면 기본값(`REASONING_EFFORT`)을 쓴다. 블로그만 올려 쓴다 — copyai.js 참고.
...
export async function generateText(prompt, opts = {}) {
  ...
  const full = { model, input: prompt, reasoning: { effort: opts.reasoningEffort || REASONING_EFFORT } };
