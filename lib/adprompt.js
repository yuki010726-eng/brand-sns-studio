...
export function buildAdPrompts({ product, topic, deck = [], conceptId, conceptIds, copyOverrides = {} }) {
   ...
   */
  // One prompt per blog image slot; rotate the four layouts before any repeat.
  const cards = deck.length ? deck : [{ kind: 'cover', title: topic, body: topic }];
  return cards.map((card, index) => {
    const cardConcept = concepts[randomIndex(`${topic}|${card.title}|${card.body}|${index}|${conceptIds?.join(",")}`, concepts.length)];
    const [line1, line2] = twoLines(card.title || topic);
    const sub = firstSentence(card.body || topic, 30);
    const override = copyOverrides[index] || {};
    const cardCopy = {
      ...copy,
      ...
      line2,
      sub,
      number: numberLine(product, [card], sub),
      ...override,
