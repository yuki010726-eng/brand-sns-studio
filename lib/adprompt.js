 ...
 *
 * ⚠️ **`desc` 는 `who`·`when` 과 같은 말투로 쓴다** (2026-08-28 3판, 요청자 지적:
 *    "지금 남성모델에 뭐라고 적혀있어? 그거 보고 그 느낌으로 따라서 적으라고").
 *
 *    화면의 그 칸에는 `30대 남성이 차분하게 설명하는 사진 · 숫자·근거로 믿음을 줘야 할 때`
 *    가 적혀 있다. **명사구로 끊고 「습니다」로 끝내지 않는다.** 그런데 `desc` 만 혼자
 *    문장체(「…모읍니다. …씁니다.」)라 한 칸 안에서 말투가 둘로 갈렸다.
 *    1판은 120~165자로 길기까지 해서 칸에 들어가지도 않았다.
 *
 *    짜임: **`…한 배너 / 그림 / 화면`** 으로 끝나는 명사구 하나, 25~30자.
 *    비교는 문장을 더 붙이지 말고 **`가장 밝은`·`가장 무거운`·`사진 없는`** 처럼
 *    수식어 한 마디로 넣는다 — 그래야 넷을 나란히 훑을 때 차이가 바로 읽힌다.
 *
 * ⚠️ 「습니다」로 끝나는 문장을 여기에 다시 쓰지 말 것. `who`·`when` 과 말투가 어긋난다.
 */
export const AD_CONCEPTS = [
  {
    ...
    desc: '크림 바탕에 노랑·빨강, 반짝임이 도는 가장 밝은 배너',
  ...
  },
  {
    ...
    desc: '왼쪽 딥네이비 패널에 글자를 모은 가장 무거운 배너',
  ...
  },
  {
    ...
    desc: '회색 고민과 노란 해결을 화살표로 잇는 사진 없는 그림',
   ...
   */
  {
    ...
    desc: '어두운 배경에 광택 오브젝트 하나만 크게 선 화면',
  ...
  /**
   * 임팩트 성과 배너 — 큰 문구를 먼저 읽히게 하는 직접반응형 광고 레이아웃.
   * 색과 주인공의 성별은 고정하지 않고, 상품의 톤과 주제에 맞춰 이미지 모델이 정한다.
   */
  {
    id: 'emblem-festival',
    name: '인증 엠블럼 행사형',
    who: '첨부한 수상·인증 엠블럼 하나를 중심에 크게 쓰는 프리미엄 행사 배너',
    when: '수상, 인증, 고객 만족도, 신뢰도처럼 공식 엠블럼이 설득의 핵심일 때',
    desc: '첨부 엠블럼의 고유 색과 대형 혜택 문구로 신뢰를 선명한 행사감으로 바꾸는 배너',
    swatch: ['#13213B', '#5F7EEA', '#F5F7FB'],
    person: false,
    usesEmblem: true,
    cast: 'no people, no food, no groceries and no product packages anywhere; the attached official award or certification emblem is the single hero visual, large, crisp and completely unobstructed in the centre',
    palette: 'build the palette from the attached emblem: use its dominant brand colour and one supporting colour, with a clean neutral background and high-contrast text. When no emblem is attached, use deep navy #13213B, periwinkle #5F7EEA and soft white #F5F7FB. Gold or red may appear only when they are already dominant colours in the attached emblem, never as a default ceremonial treatment',
    art: 'a polished Korean premium direct-response event poster with crisp geometric light, subtle tonal texture and a modern editorial ceremony atmosphere; luxurious, energetic and high-contrast, never a food advertisement. Let the attached emblem determine the visual tone rather than defaulting to gold foil, red ribbons, laurels or medal imagery',
    frame: [
      'a compact angled colour block drawn from the emblem palette spans part of the top edge and holds the speech-bubble hook in high-contrast Korean lettering',
      'the attached emblem occupies the centre and is the only recognisable hero image, filling about forty percent of the canvas height with generous clear space around its silhouette',
      'the headline sits directly below the emblem in two huge stacked lines, centred, with a clean high-contrast outline and restrained shadow in colours derived from the emblem',
      'the supporting line sits in a narrow rule area using the supporting palette colour',
      'the big number line or event-benefit line dominates the lower third in the primary emblem colour, with a light outline and a subtle dark shadow',
      'the check items are small rounded labels arranged in one neat row above the bottom CTA bar',
      'a full-width CTA bar in the primary emblem colour anchors the very bottom, with high-contrast lettering',
    ],
    prop: 'as small abstract geometric accents in the emblem palette only; do not add another product, package, bottle, box, food, ingredient or invented logo',
  },
  {
    id: 'impact-success-banner',
    name: '임팩트 성과 배너',
    who: '성과를 만들거나 문제를 해결한 인물·캐릭터 일러스트',
    when: '교육·서비스·프로모션처럼 강한 행동 유도가 필요할 때',
    desc: '초대형 헤드라인과 대각선 그래픽으로 메시지를 강하게 각인하는 배너',
    swatch: ['#FFFFFF', '#287AFF', '#111827'],
    person: true,
    cast: 'a confident, approachable Korean person appropriate to the topic, any gender and age appropriate to the audience, shown waist-up with a positive achievement gesture; use a friendly Korean commercial illustration character instead of a person when the topic calls for it',
    palette: 'a clean light background with a high-contrast primary colour and a darker typography colour, chosen to suit the product or brand visual tone; use one small accent colour only for emphasis',
    art: 'polished Korean commercial illustration with crisp outlines, dynamic diagonal paint-brush strokes, subtle halftone texture, and small achievement symbols such as check marks, progress arrows, benefit cards or communication icons',
    frame: [
      'an angled ribbon spans the upper area and contains a short hook phrase',
      'the main headline dominates the upper and middle left area in three or four huge stacked lines of extra-bold Korean gothic typography, taking up most of the card',
      'a short supporting sentence sits below the headline with one key phrase highlighted in the primary colour',
      'a dynamic diagonal painted graphic field rises from the lower right with small achievement symbols, leaving the headline unobstructed',
      'the main person or character appears prominently in the lower right, waist-up, overlapping the graphic field with a confident positive gesture',
      'a row of three small topic-relevant product, feature or service icons sits along the lower left edge',
    ],
    prop: 'as three small cleanly illustrated topic-relevant product, feature or service icons aligned in a row along the lower left edge',
  },
];
 ...
 */
export function buildAdPrompts({ product, topic, deck = [], conceptId, conceptIds, copyOverrides = {} }) {
  const selectedConcepts = (Array.isArray(conceptIds) ? conceptIds : [conceptId])
    .map((id) => AD_CONCEPTS.find((item) => item.id === id))
    .filter(Boolean);
  const concepts = selectedConcepts.length ? selectedConcepts : [getAdConcept(conceptId)];
  // A seeded draw keeps a prompt stable while the user edits copy, while still
  // spreading cards across the templates they selected.
  const randomIndex = (seed, size) => {
    let hash = 2166136261;
   ...
   */
  // An advertising sub-template produces one finished banner, not one banner
  // for every card in the content deck.  Using the lead card supplies the
  // strongest copy while preventing a five-card deck from creating five
  // near-duplicate images for the same selected sub-template.
  const card = deck[0] || { kind: 'cover', title: topic, body: topic };
  const cardConcept = concepts[randomIndex(`${topic}|${card.title}|${card.body}|${conceptIds?.join(",")}`, concepts.length)];
  const [cardLine1, cardLine2] = twoLines(card.title || topic);
  const cardSub = firstSentence(card.body || topic, 30);
  const override = copyOverrides[0] || {};
  const cardCopy = {
    ...copy,
    hook: hookOf(product, [card]),
    line1: cardLine1,
    line2: cardLine2,
    sub: cardSub,
    number: numberLine(product, [card], cardSub),
    ...override,
    bullets: Array.isArray(override.bullets) ? override.bullets : copy.bullets,
  };
  return [{
    n: 1,
    role: card.kind || 'single',
    concept: cardConcept,
    copy: cardCopy,
    prompt: promptOf({ product, concept: cardConcept, copy: cardCopy, subject: subjectOf([card], topic, cardConcept.prop, cardCopy.propLabel) }),
  }];
}
...
function promptOf({ product, concept, copy, subject }) {
  ...
  return [
    ...
    ...(concept.usesEmblem ? [
      'REFERENCE EMBLEM: An official emblem image is attached as a visual reference. Use that exact attached emblem as the one and only official mark: preserve its original shape, lettering, colours and proportions. Do not redraw, simplify, translate, crop, alter or replace it. Keep it crisp, large, fully visible and unobstructed.',
      'The attached emblem must be included as the central hero visual and must set the banner palette. If no emblem image is attached, do not invent an award seal, certificate, medal, logo or badge; leave the central hero area to the written headline and subtle geometric light only.',
    ] : []),
...
export function adThumbSvg(c, { size = 120, id = '' } = {}) {
  ...
  const subject = c.id === 'emblem-festival'
    ? `<circle cx="76" cy="47" r="19" fill="#17120B" stroke="${key}" stroke-width="3" />
       <path d="M64 64l-5 15 17-8 17 8-5-15" fill="${key}" opacity=".9" />
       <path d="M70 43h12M70 49h12" stroke="#FFF4D6" stroke-width="2" stroke-linecap="round" />`
    : c.person
