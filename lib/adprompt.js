...
export const AD_CONCEPTS = [
  {
    id: 'woman-yellow',
    name: '밝은 여성 · 옐로우',
    desc: '20대 후반 한국 여성 모델. 크림 배경에 옐로우·레드. 말풍선과 체크리스트가 잘 붙습니다.',
    person: true,
    cast: 'the same Korean woman in her late twenties throughout: dark brown hair pulled into a loose bun with soft bangs, warm open smile with visible teeth, wearing a mustard-yellow ribbed knit sweater, expressive raised-index-finger gestures',
  ...
  },
  {
    id: 'man-navy',
    name: '신뢰감 남성 · 네이비',
    desc: '30대 후반 한국 남성 모델. 화이트 배경에 네이비·옐로우. 숫자와 근거를 세울 때 좋습니다.',
    person: true,
    cast: 'the same Korean man in his late thirties throughout: short neatly-parted black hair, calm confident half-smile, wearing a white dress shirt under a navy blazer with no tie, open-palm presenting gestures',
  ...
  },
  {
    id: 'duo-cartoon',
    name: '2인 만화형',
    desc: '고민하는 사람과 안내하는 사람. 두 칸 비교나 질문–답변 구성에 맞습니다.',
    person: true,
    cast: 'the same two Korean cartoon characters throughout: a worried office worker in a grey shirt with round glasses, and a cheerful advisor in a yellow cardigan with a clipboard; keep both faces, hair, and outfits identical in every image',
  ...
  },
  {
    id: 'scene-real',
    name: '실사 장면 · 인물 없음',
    desc: '실제 매장·사무실 사진 위에 글자. 제품과 현장을 보여줄 때 씁니다.',
    person: false,
    cast: 'no people in frame; the same warm, bright Korean small-business interior throughout — pale wood counter, white subway tile, black pendant lights, a few potted plants — shot from the same angle and lighting in every image',
  ...
  },
  {
    id: 'icon-flat',
    name: '플랫 아이콘 · 인물 없음',
    desc: '인물 없이 아이콘과 도형만. 절차·항목 정리에 잘 맞습니다.',
    person: false,
    cast: 'no people and no photographs anywhere; the same flat vector icon language throughout — rounded 2px strokes, solid fills, consistent corner radius and icon weight in every image',
 ...
 *    - **덱보다 많이** 뽑으면 재료가 모자라 반복이 생긴다. 이건 문구를 지어내지 않는 한
 *      피할 수 없다 — 그래서 감추지 않고 `capacityOf()` 로 화면에 알린다.
 */
...
/**
 * 이 덱으로 **내용이 서로 다른** 카드를 최대 몇 장까지 만들 수 있나.
 *
 * 덱 한 장이 카드 한 장의 재료다. 이보다 많이 뽑으면 본문 장면이 반복된다 —
 * 화면에서 이 숫자로 안내한다 (`pages/template.js`).
 */
export const capacityOf = (deck) => (Array.isArray(deck) ? deck.length : 0);

/* ---------------- 프롬프트 ---------------- */

/**
 * @param {{product:object, topic:string, deck:Array, count:number, conceptId:string}} ctx
 * @returns {Array<{n:number, role:string, concept:object, copy:object, prompt:string}>}
 */
export function buildAdPrompts({ product, topic, deck = [], count = DEFAULT_AD_COUNT, conceptId }) {
...
function promptOf({ product, concept, role, copy, subject, index, total }) {
  ...
  return [
    'square 1:1 Korean direct-response social ad card',
    ...(total > 1 ? [
      `SERIES: card ${index + 1} of ${total} in one set. Every card in this set uses the same subject, the same colors, and the same graphic style — keep them identical across the set. Only the message and the object being shown change from card to card.`,
    ] : []),
    `SUBJECT (identical on every card): ${concept.cast}`,
    `WHAT THIS CARD SHOWS: ${subject}`,
    `LAYOUT: ${ROLE_FRAME[role].join(', ')}`,
    `COLOR PALETTE (identical on every card): ${concept.palette}`,
    `STYLE (identical on every card): ${concept.art}`,
