...
export const AD_CONCEPTS = [
  ...
  },
  {
    ...
    person: false,
    /**
     * ⚠️ **2026-08-28 — 「촌스러워지고 밋밋해졌다」** (요청자 지적). 원인은 `art` 의
     *    `no gradients` · `simple geometric shapes` 였다. 둘이 겹치면서 아이콘이 **단색 실루엣**이
     *    되고, 화면이 색 면 몇 개로 남았다. 요청자가 좋다고 한 판은 아이콘마다 색이 다르고
     *    부드러운 음영과 하이라이트가 있었다 — 납작한 것과 밋밋한 것은 다르다.
     * ⚠️ `no gradients` 를 되살리지 말 것. 그 한 줄이 아이콘에서 입체감을 통째로 뺀다.
     */
    /**
     * ⚠️ **2026-08-28 2판 — 「매력 없는 디자인의 표본」** (요청자 지적). 1판에서 `no gradients` 를
     *    걷어냈는데도 아이콘이 **뜻 없는 추상 도형**(색색 둥근 사각형 + 화살표)으로 나왔다.
     *    「richly illustrated」 만으로는 무엇을 그릴지 정해 주지 않아서다.
     *
     *    요청자가 좋다고 한 판은 아이콘이 전부 **알아볼 수 있는 물건**이었다 — 리본 달린 액자
     *    상장 · 월계관 두른 금 트로피 · 사진이 박힌 신문. 그리고 흰 패널·굵은 테두리·노란 알약
     *    라벨·헤드라인 둘레의 짧은 노란 빗금까지 **장식이 있었다.**
     *
     * ⚠️ **아이콘은 그 아래 체크 문구가 말하는 물건을 그린다.** 이 한 줄이 추상 도형을 막는다.
     * ⚠️ 장식(빗금·반짝임·굵은 흰 외곽선·그림자)을 빼지 말 것. 그게 빠지면 색 면만 남는다.
     */
    cast: 'no people and no photographs anywhere; every icon is a recognisable everyday object drawn as a rich illustration with thick outlines, soft shading and a gentle highlight, each in its own bright colour, and each icon depicts the very object named in the check item beneath it rather than an abstract shape or a plain silhouette',
    palette: 'background white #FFFFFF, primary deep green #1F7A45, alert red #E8391F, highlight yellow #FFD84D, text near-black #111111, colour laid down in bold solid blocks and filled pills rather than thin outlines',
    art: 'bold sticker-style graphic design — headline words set inside solid colour blocks with a thick white outline and a soft drop shadow, short yellow slash marks and small sparkles scattered around the headline as decoration, icons resting inside white rounded panels with thick coloured borders, generous white space',
    /**
     * 가로 밴드 + 3칸 그리드 구도 — 「절차·항목을 정리해 보여줄 때」 쓰는 컨셉이다.
     *
     * ⚠️ **인물이 없으므로 좌우로 나누지 않는다.** 다른 셋은 오른쪽에 인물이 서서 왼쪽 60%가
     *    글자 자리가 되는데, 여기서 그 배치를 쓰면 오른쪽 절반이 통째로 빈다.
     *    위아래 밴드로 쌓고 **가운데를 아이콘 3칸이 채운다** — 그게 항목 정리라는 성격에도 맞는다.
     */
    frame: [
      'no photographs and no person anywhere, so the whole card is built as stacked horizontal bands rather than a left-and-right arrangement',
      ...
      'three equal white rounded panels with thick coloured borders sit side by side across the middle of the frame, each holding one large richly illustrated icon above a filled pill label carrying its check item',
    ],
   ...
   * 입체 오브젝트**(확성기·자명종) + 극태 헤드라인 + 하단 둥근 흰 카드. 아이콘 그림(`icon-flat`)과
   * 헷갈리기 쉬운데 갈리는 지점이 분명하다 —
   *   `icon-flat` : 밝은 배경 · 납작한 그림 · 항목을 **정리**해 보여준다
   *   `icon-3d`   : 어두운 배경 · 광택 입체 · 하나를 **크게 띄워** 눈길을 끈다
   *
