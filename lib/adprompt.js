...
export const AD_CONCEPTS = [
  {
    ...
    desc: '크림 바탕에 노랑과 빨강을 얹은 가장 밝은 배너입니다. 오른쪽에서 인물이 말을 걸고 왼쪽 60%를 헤드라인이 채웁니다. 반짝임과 방사선으로 시선을 붙잡기 때문에, 이 상품을 처음 듣는 사람에게 알리는 글에서 가장 셉니다. 대신 무게가 실리지 않으니 따져 읽어야 하는 내용에는 남성 모델을 쓰세요.',
  ...
  },
  {
    ...
    desc: '왼쪽에 딥네이비 패널 한 장을 세우고 글자를 전부 그 안에 넣습니다. 무게는 색이 아니라 정렬에서 나오므로 노랑은 작은 강조에만 쓰고 반짝임은 넣지 않습니다. 심사 기준·과금 방식·계약 조건처럼 독자가 따져 읽는 내용에 맞습니다.',
  ...
  },
  {
    ...
    desc: '아래를 회색 면과 노란 면으로 갈라 왼쪽에 고민, 오른쪽에 해결을 놓고 가운데를 화살표가 건넙니다. 사진이 없어 광고 티가 덜 나고, 「우리 업종도 되나」처럼 독자가 먼저 품는 반론을 정면으로 받는 주제에 맞습니다.',
   ...
   */
  {
    ...
    desc: '어두운 배경에 주인공 오브젝트 하나를 크게 세우고, 나머지는 반짝임·번개 같은 작은 도형으로만 꾸밉니다. 인물이 없어 업종을 안 타는 것이 강점입니다. 말하려는 것이 물건이나 화면 하나로 좁혀질 때 쓰고, 주인공이 뚜렷하지 않은 주제에는 맞지 않습니다.',
