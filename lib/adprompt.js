 ...
 */
export function buildAdPrompts({ product, topic, deck = [], conceptId, copyOverrides = {} }) {
   ...
   */
  const firstIndex = Math.max(0, AD_CONCEPTS.findIndex((item) => item.id === concept.id));
  ...
  return cards.map((card, index) => {
    const cardConcept = AD_CONCEPTS[(firstIndex + index) % AD_CONCEPTS.length];
