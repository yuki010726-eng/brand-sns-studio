...
/** 이미지 주제 설정에서 새로 선택할 수 있는 카드뉴스 템플릿 순서. */
export const IMAGE_TOPIC_CONCEPT_IDS = ["intuitive", "card", "note", "magazine"];
...
/**
 * 매거진형 세부 템플릿 4종 (2026-09-02, 피그마 node 206-16/206-23/206-36 반영).
 *
 * 매거진형은 표지·본문·마무리로 여러 장을 만들던 것을 그만두고 **표지 한 장만** 만든다.
 * 그 한 장의 배치를 고르는 자리가 이 템플릿이다 — 예전에 카드 탭(01~06)이 있던 자리를
 * 이 선택지가 대신한다 (`app/template/page.jsx`).
 *
 * - `t1` : 기존 매거진형 배치 그대로, 우상단 페이지 번호({현재 장}/{전체 장})만 뺐다.
 *          한 장만 만드니 '몇 번째 장'이라는 숫자 자체가 뜻이 없어졌다.
 * - `t2` : 왼쪽 정렬. 계정 이름 옆에 가로 구분선, 제목은 위쪽 고정 위치.
 * - `t3` : 가운데 정렬. 구분선 없음, 제목은 t1처럼 아래쪽.
 * - `t4` : 가운데 정렬. 계정 이름과 제목 사이에 짧은 세로 구분선, 제목은 위쪽 고정 위치.
 *
 * `t2`·`t4`의 구분선은 렌더러(`lib/cardrender.js`)가 그리되, 위치·길이는 `layout.divider`
 * 오버라이드로 자유롭게 옮기고 줄이고 지우고 다시 만들 수 있다 — 텍스트 오브젝트의
 * 자유 배치(`layout[id]`)와 같은 방식이다.
 */
export const MAGAZINE_TEMPLATES = [
  { id: "t1", name: "템플릿 1", hasDivider: false },
  { id: "t2", name: "템플릿 2", hasDivider: true },
  { id: "t3", name: "템플릿 3", hasDivider: false },
  { id: "t4", name: "템플릿 4", hasDivider: true },
];

export const DEFAULT_MAGAZINE_TEMPLATE = MAGAZINE_TEMPLATES[0].id;

/** @param {string} id */
export const getMagazineTemplate = (id) =>
  MAGAZINE_TEMPLATES.find((t) => t.id === id) || MAGAZINE_TEMPLATES[0];

/**
 * 카드형 우상단 마크.
 * 계정 성격에 맞춰 고를 수 있게 후보를 둔다. `id` 는 lucide 아이콘 이름과 같아서
 * 화면 picker(`Icon` 컴포넌트)와 렌더러(`lib/cardrender.js` 의 `MARK_ICON_PATHS`)가
 * 같은 값으로 아이콘을 찾는다 — 하나만 바꾸면 그림과 picker 가 어긋난다.
 * glyph 는 시스템 이모지 폰트로 그린다 (지금은 쓰지 않지만 렌더러 경로는 살아 있다).
 */
/**
 * ⚠️ 2026-08-31, 요청자 지시로 도형 마크(별표·반짝임·별·점·십자)를 목적 기반 5종으로 바꿨다.
 *    없앤 id 가 저장돼 있어도 `getMark()` 가 기본값(MARKS[0])으로 되돌린다.
 */
export const MARKS = [
  { id: "newspaper", name: "뉴스" },
  { id: "megaphone", name: "홍보" },
  { id: "shield-check", name: "신뢰" },
  { id: "circle-fading-plus", name: "콘텐츠" },
  { id: "trending-up", name: "성과" },
  { id: "none", name: "없음" },
];
...
export const getNotePaper = (id) =>
  NOTE_PAPERS.find((x) => x.id === id) || NOTE_PAPERS[0];
...
/** @param {string} id */
export const getNoteSymbol = (id) =>
  NOTE_SYMBOLS.find((s) => s.id === id) || NOTE_SYMBOLS[0];
...
export const CONCEPTS = [
  {
    ...
    // 템플릿 선택 화면(ConceptPicker) 미리보기 카드 이미지. 비어 있으면 자리만 표시한다.
    previewImages: [
      "/concept-preview/magazine-1.webp",
      "/concept-preview/magazine-2.webp",
      "/concept-preview/magazine-3.webp",
      "/concept-preview/magazine-4.webp",
    ],
    accentPicker: true, // 강조 색상을 고를 수 있는 유일한 컨셉
    /**
     * ⚠️ **2026-08-28 교체 — 명세는 `MAGAZINE_IMAGE_PROMPT.md` 3절.**
     *
     * 요청자 지적 둘에서 출발했다 — 「어떤 주제를 넣어도 비슷해 보인다」, 「너무 어둡고 누렇게 나온다」.
     * 이전 값에서 뺀 것과 이유:
     *   `cinematic` · `dramatic directional lighting`  무드 형용사가 톤을 묶어 **전 장이 같은 사진**이 됐다
     *   `wide dynamic composition`                     구도는 장면(shot)이 정한다. 두 곳에서 지시하면 싸운다
     *   `darker tone toward the bottom`                렌더러가 이미 하단 그라데이션을 그린다 — **이중으로 어두워졌다**
     * 새로 넣은 것:
     *   `neutral white balance, no overall warm or amber color grading`  색온도 지시가 없으면 모델이 금색으로 간다
     *   `strong tonal contrast … premium materials`                      금색에 기대지 않고 고급감을 낸다
     *
     * ⚠️ **무드 형용사와 밝기(`bright`·`open shadows`)를 여기에 되살리지 말 것.** 둘 다 실측으로 확인했다 —
     *    무드를 넣으면 모든 주제가 같은 사진이 되고, 밝기를 넣으면 그림자가 사라져 싸구려 사무실 사진이 된다.
  ...
  },
  {
    ...
    previewImages: [
      "/concept-preview/card-1.webp",
      "/concept-preview/card-2.webp",
      "/concept-preview/card-3.webp",
      "/concept-preview/card-4.webp",
    ],
    /**
     * ⚠️ **2026-08-28 교체 — 명세는 `CARD_IMAGE_PROMPT.md` 2절.**
     *
     * 이전 값에서 뺀 것과 이유:
     *   `open sky or wide scene`      주제가 서류·실내여도 **하늘과 야외로 끌려갔다.** 야외가 맞으면 장면에 직접 쓴다
     *   `cool blue leaning grading`   렌더러가 이미 하단 파랑 그라데이션을 그린다 — **파란 사진 위에 파랑이 얹혀** 한 색으로 눌렸다
     *   `generous empty space …`      `SPACE[kind]` 가 이미 여백을 말한다. 같은 지시가 두 곳에 있었다
     *
     * ⚠️ **`bright` 는 유지한다.** 매거진형에서는 뺐지만 카드형은 밝은 것이 정체성이다.
     * ⚠️ **`cool blue` 를 되살리지 말 것.** 카드 전체가 한 색으로 눌린다.
     * ⚠️ 밝은 톤은 여백이 그대로 빈틈으로 읽힌다 — 화면을 더 채워야 한다(`CARD_IMAGE_PROMPT.md` 3절).
     *    채우는 것은 **장면(shot)** 이 할 일이라 여기에 적지 않는다.
     */
    style: [
  ...
  },
  {
    ...
    previewImages: [
      "/concept-preview/note-1.webp",
      "/concept-preview/note-2.webp",
      "/concept-preview/note-3.webp",
      "/concept-preview/note-4.webp",
    ],
    style: [
  ...
  },
  {
    ...
    badge: "D",
    name: "블로그형",
    ref: "블로그형 레퍼런스 (요청자 제공)",
    desc: "주제와 브랜드에 맞는 색·구도로 기사, 인증, 수상, 안내 정보를 또렷하게 전달합니다.",
    mood: "공식적이고 신뢰감 있는 안내. 수상, 인증, 보도자료, 서비스 절차에 어울립니다.",
    previewImages: [
      "/concept-preview/blog-1.webp",
      "/concept-preview/blog-2.webp",
      "/concept-preview/blog-3.webp",
      "/concept-preview/blog-4.webp",
    ],
    style: [
  ...
  /**
   * 광고형 — **다른 셋과 성격이 다르다.** 카드를 그리지 않고 이미지 프롬프트만 만든다.
   *
   * ⚠️ **2026-08-28 이름을 바꿨다: 직관형 → 광고형** (요청자 지시). 「직관형」은 이 컨셉이
   *    무엇인지 말해 주지 않는다 — 셋과 갈리는 지점은 직관이 아니라 **완결된 광고 배너**라는
   *    것이다. 코드의 id 는 `intuitive` 그대로 둔다. 보관본과 저장된 상태가 그 값을 갖고 있어서
   *    바꾸면 옛 게시물이 매거진형으로 떨어진다.
   *
   ...
   *
   * ⚠️ `promptOnly` 를 보고 화면이 통째로 갈아 끼워진다. 이 값을 빼면 캔버스 편집 화면이
   *    뜨는데, 광고형에는 슬롯 정의(`lib/templates.js`)가 없어서 매거진형 슬롯으로 떨어진다 —
   *    고장 난 것처럼 보인다.
   * ⚠️ `style` 은 비워 둔다. `lib/imageprompt.js` 는 광고형을 그리지 않는다.
   */
  {
    id: "intuitive",
    badge: "E",
    ...
    previewImages: [
      "/concept-preview/ad-1.webp",
      "/concept-preview/ad-2.webp",
      "/concept-preview/ad-3.webp",
      "/concept-preview/ad-4.webp",
      "/concept-preview/ad-5.webp",
      "/concept-preview/ad-6.webp",
    ],
    promptOnly: true,
    style: "",
    layout: {
...
export const getConcept = (id) =>
  CONCEPTS.find((c) => c.id === id) || CONCEPTS[0];

/** 매거진형만 강조 색상을 쓴다. 나머지는 자기 레이아웃 색을 유지한다. */
export function accentOf(concept, chosen) {
  if (!concept.accentPicker)
    return concept.layout.accent || concept.layout.brand || "#FFFFFF";
