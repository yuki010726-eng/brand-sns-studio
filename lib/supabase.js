/**
 * Supabase 연결 — 구글 로그인과 작업 내용 동기화의 토대
 *
 * 왜 필요한가
 ...
 *
 * ⚠️ **설정이 없어도 앱은 완전히 동작해야 한다.** 규칙 기반 글귀를 남겨 둔 것과 같은 원칙이다.
 *    로그인은 '있으면 좋은 것'이지 앱을 막는 관문이 아니다.
 */
 ...
 *
 * 값이 비어 있으면 로그인 기능만 꺼지고 나머지는 그대로 돌아간다.
 */
    ...
    clientPromise = import(/* @vite-ignore */ SDK_URL)
      .then(({ createClient }) => createClient(SUPABASE.url, SUPABASE.anonKey, {
        auth: {
          ...
          detectSessionInUrl: true,   // 구글에서 돌아왔을 때 토큰을 주소에서 회수한다
        },
      }))
      .catch((e) => {
        // 네트워크가 막힌 환경에서도 앱은 계속 써야 한다
        console.warn('[supabase] SDK 로드 실패 — 로그인 없이 계속합니다.', e);
        clientPromise = null;
        return null;
      });
  }
