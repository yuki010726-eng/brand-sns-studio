...
import { pullLibraryPosts, removePostMetadata, savePostMetadata } from './sync.js';
...
export const POST_KEYS = [
  'productId', 'topic', 'focusPoint', 'contentOutline', 'libraryTitle', 'tone', 'customStyleUrl', 'customStyleGuide', 'customStyleGuideUrl', 'customStyleSaveRequested', 'channels', 'cardCount',
  'drafts', 'generated', 'variants', 'sources', 'draftKey', 'aiKey', 'outline', 'cardCopy', 'researchStyle',
  'aiRuns', 'activeAiRun',
  'concept', 'concepts', 'magazineTemplate', 'accent', 'mark', 'cardTheme', 'noteSymbol', 'notePaper', 'noteGrain', 'blogConcept', 'blogConcepts',
  'card', 'image', 'images', 'postId', 'blogImageLayout',
];
 ...
 */
export async function saveToLibrary(state, thumbBlob = null, { type = 'text_image' } = {}) {
  ...
  const item = {
    ...
    // Keep the original creation flow when editing a saved post.
    type: at >= 0 && before[at].type === 'image' ? 'image' : (type === 'image' ? 'image' : 'text_image'),
  ...
  const remote = await savePostMetadata(state, id, item);
  if (!remote.ok) {
    console.warn('[library] saved locally, but post metadata sync failed', remote.error || 'not signed in');
  }
...
export async function removeFromLibrary(id) {
  ...
  const remote = await removePostMetadata(id);
  if (!remote.ok) console.warn('[library] removed locally, but server deletion failed', remote.error || 'not signed in');
}

/**
 * Merge the signed-in user's cloud library into this device.  Locally saved
 * thumbnails are retained; for the same post the newest edit wins.
 */
export async function pullLibraryFromCloud() {
  const remote = await pullLibraryPosts();
  if (!remote.ok) return remote;
  const local = getLibrary();
  const localById = new Map(local.map((item) => [item.id, item]));
  const merged = new Map(local.map((item) => [item.id, item]));
  for (const cloudItem of remote.items) {
    const deviceItem = localById.get(cloudItem.id);
    const cloudIsNewer = !deviceItem
      || String(cloudItem.updatedAt || '') > String(deviceItem.updatedAt || '');
    if (cloudIsNewer) {
      merged.set(cloudItem.id, {
        ...cloudItem,
        hasThumb: Boolean(deviceItem?.hasThumb),
      });
    }
  }
  const library = [...merged.values()].sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)));
  setState({ library });
  return { ok: true, items: library };
}
...
export async function loadFromLibrary(id) {
  ...
  const hasSavedCardCopy = Object.prototype.hasOwnProperty.call(snap, 'cardCopy');
  ...
  // Older library snapshots did not include `cardCopy`.  On those entries,
  // the template page rebuilt the card from whichever copy was already in the
  // global store (often the image-only flow) and silently replaced the saved
  // card's generated text.  Treat every stored text field as intentional for
  // that legacy shape so `reconcileCard` keeps the card the user saved.
  if (!hasSavedCardCopy && Array.isArray(snap.card?.texts)) {
    snap.card = {
      ...snap.card,
      base: snap.card.texts.map(() => ({})),
    };
  }
