...
import { savePostMetadata } from './sync.js';
...
export async function saveToLibrary(state, thumbBlob = null) {
  ...
  // Keep Supabase compact: it receives only the saved-post metadata, while the
  // full editable document stays in this device's local library.
  const remote = await savePostMetadata(state, id);
  if (!remote.ok) {
