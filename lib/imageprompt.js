...
import { getBlogConcept } from './blogprompt.js';
 ...
 */
const SPACE = {
  cover: 'the lower third of the frame free of clutter and important detail so text can be placed there',
  body: 'a simple composition with a calm area in the middle of the frame for a text panel',
  note: 'a simple composition with a calm area in the middle of the frame for a text panel',
...
export function buildPrompt(card, conceptId, opts = {}) {
  ...
  /**
   * ⚠️ **2026-09 — 명령문으로 열면 "편집 요청"으로 잘못 판정된다.** 예전 이론(위 주석,
   * 삭제됨)은 "편집·업로드·참조 이미지 같은 낱말을 빼면 된다"였는데, 그 낱말을 다 뺐는데도
   * 실제로 계속 편집으로 잡혔다. `lib/adprompt.js` 의 2026-08-28 3판이 진짜 원인을 찾았다 —
   * **낱말이 아니라 문장의 생김새**다. `Generate …` `Start …` `Render …` `Do not …` 처럼
   * **명령문으로 시작하거나 명령문이 곳곳에 섞이면**, 이미지 라우터가 "무엇을 그릴지 설명하는
   * 글"이 아니라 "기존 그림에 무엇을 하라는 지시"로 읽는다.
   *
   * 그래서 여기서는 **명령문 오프닝을 넣지 않는다.** 첫 줄은 항상 화풍(`concept.style` /
   * `blogConcept.style`)이나 `noteIconPrompt` 의 명사구로 바로 시작하고, "하지 마세요·절대·
   * avoid" 류는 전부 맨 끝 `negative:` 한 줄로 모은다(`negativeParts`). `negative:` 형태는
   * text-to-image 쪽 관습이라 오히려 "새로 생성해라"는 신호로 읽힌다.
   * ⚠️ **명령문 오프닝을 되살리지 말 것.** 되살리면 이 문제가 그대로 돌아온다.
   */
  if (concept.id === 'note') return noteIconPrompt(card, opts.title, opts.subject);

  // 블로그형은 카드뉴스(인스타 세로형)가 아니라 블로그 본문에 들어가는
  // 가로 배너다. 장별로 다른 편집 구도를 써서 레퍼런스의 분위기는 살리되
  // 특정 좌우 분할 구도에 고정되지 않게 한다.
  const isBlog = concept.id === 'blog';
  const blogConcept = isBlog ? getBlogConcept(opts.blogConceptId) : null;
  const isPeopleStory = isBlog && Boolean(blogConcept?.peopleOnly);
  const isIllustratedPolicy = isBlog && blogConcept?.id === 'illustrated-policy';
  const articleContext = String(
    opts.articleContext || [opts.title || card.title, opts.body || card.body].filter(Boolean).join('\n'),
  ).trim();
  // A short `shot` can be useful framing metadata, but for the people-story
  // template it must never overrule the article section it is illustrating.
  const subject = isPeopleStory && articleContext ? articleContext : opts.subject;
  const blogCopy = isBlog
    ? [
        ['small eyebrow', opts.eyebrow],
        ['main Korean headline', opts.title || card.title],
        ['supporting Korean copy', opts.body || card.body],
      ]
        .filter(([, value]) => String(value || '').trim())
        .map(([label, value]) => `${label}: "${String(value).trim()}"`)
        .join('; ')
    : '';
  // 블로그 배너가 매번 '왼쪽 텍스트 + 오른쪽 제품 사진'으로 굳어지지 않도록,
  // 장마다 서로 다른 편집 구도를 쓴다. 색은 의도적으로 여기서 정하지 않는다.
  const blogLayouts = blogConcept?.layouts || [
  ...
  const blogLayout = blogLayouts[Math.abs(Number(opts.index) || 0) % blogLayouts.length];
  const blogComposition = isPeopleStory
    ? [
        'TEMPLATE MODE: PEOPLE-AT-WORK STORY PHOTO.',
        `documentary on-location photograph: ${blogLayout}`,
        'the people doing the real work are the unmistakable main subject and occupy most of the frame, in a specific service or craft in progress with meaningful interaction between people, hands, tools, and the relevant work area',
      ].join(' ')
    : isIllustratedPolicy
      ? [
          'TEMPLATE MODE: ILLUSTRATED POLICY EXPLAINER.',
          `2D Korean public-service editorial illustration: ${blogLayout}`,
          'a designed hand-drawn illustration — never a photograph, a photographed scene, a realistic 3D render, or a corporate editorial photo — with simplified but expressive Korean/East Asian characters, clean dark-navy outlines, flat or gently textured pastel color areas, and diagram-like visual storytelling',
          "a clear visual explanation that turns the article's people, service, problem, change, and outcome into a connected scene or a few intentional illustrated vignettes, in the sky-blue, navy, teal, green, warm yellow, and small coral palette from the template",
        ].join(', ')
      : `landscape Korean corporate PR campaign composition: ${blogLayout}, with a palette that the subject, brand character, and lighting determine rather than a preset navy-and-white, blue-and-white, or corporate color scheme`;

  /**
   * ⚠️ **"하지 마세요·avoid·never" 는 전부 여기로 모은다** (위 함수 머리의 2026-09 주석 참고).
   * 본문 곳곳에 흩어 두면 문장마다 명령문이 되어 편집 판정을 다시 부른다.
   * 순서는 위 본문이 나열한 순서와 같게 둔다 — 나중에 원인을 찾을 때 대응이 쉽다.
   */
  const negativeParts = [
    isIllustratedPolicy ? 'camera, lens, photographic, realistic, or on-location framing language' : '',
    isPeopleStory ? 'a corporate PR banner, award visual, press-release graphic, product hero shot, empty showroom, or posed team portrait' : '',
    isIllustratedPolicy ? 'glossy press-release aesthetics or heavy photo-like lighting' : '',
    'generic stock-photo scenes, a person posing thoughtfully, a meeting room, or a laptop on a desk unless the scene specifically calls for it',
    isPeopleStory ? 'an award, certificate, newspaper, press material, information graphic, polished display object, or isolated product shown unless it is physically being used by the people in the real work scene' : '',
    isPeopleStory
      ? 'any readable text — headlines, captions, signage, labels, posters, UI text, logos, watermarks, numbers, typography, graphic panels, collages, or split-screen layouts'
      : isBlog
      ? 'a fixed navy-and-white palette, a fixed left-copy/right-visual layout, Instagram handles, @mentions, social-media account names, footer handles, placeholder text, gibberish, watermarks, logos, or any words beyond the banner text above'
      : '',
    isPeopleStory && articleContext ? 'using this article content merely as a mood reference, unrelated generic workers, or any part of this text rendered in the image' : '',
    subject && (isPeopleStory || !isBlog) ? 'the subject sentence rendered as text in the image' : '',
    isIllustratedPolicy ? 'photography, photorealistic people, camera depth of field, glossy product rendering, stock-photo composition, or editorial photo treatment' : '',
  ].filter(Boolean);

  return [
    blogConcept?.style || concept.style,
    isIllustratedPolicy
      ? 'illustrated Korean/East Asian people and places when relevant'
      : isPeopleStory
      ? 'all people are Korean with East Asian faces, shot on location in South Korea'
      : localeFor(card.shot),
    isBlog
      ? blogComposition
      : SPACE[card.kind] || SPACE.body,
    /**
     * ⚠️ **이 지시를 빼지 말 것** (2026-08-20).
     ...
     * (제외 목록 자체는 `negativeParts` 로 옮겼다 — 여기서 부정문으로 다시 쓰지 말 것.)
     */
    isBlog
      ? 'horizontal 3:2 landscape composition, wide blog banner, never vertical or square'
      : 'vertical 4:5 portrait composition',
    isPeopleStory
      ? 'a clean text-free editorial photograph, not a designed banner'
      : isBlog
      ? `the Korean text placed naturally inside the composition, exactly as written with correct Hangul spelling and line breaks chosen for a polished banner layout, in a typography color, background treatment, and information-panel treatment chosen for legibility and the subject's own visual character: ${blogCopy}`
      : '',
    card.shot,
    isPeopleStory && articleContext
      ? `ARTICLE CONTENT TO ILLUSTRATE, whose concrete situation, service, problem, outcome, and people this photograph shows through the people actively doing or experiencing it: "${articleContext}"`
      : '',
    subject
      ? `THE SUBJECT THAT MATTERS MOST, whose concrete object or action the visual centers on even when the style above pulls toward a generic mood: "${subject}"${isPeopleStory ? ', told through people visibly doing the relevant work' : isBlog ? ' (the banner text above is the only text allowed in the image)' : ''}`
      : '',
    isIllustratedPolicy
      ? 'medium: an original 2D Korean policy-campaign illustration with visible drawn outlines and simplified color shapes'
      : '',
    negativeParts.length ? `negative: ${negativeParts.join(', ')}` : '',
  ].filter(Boolean).join('. ');
}
