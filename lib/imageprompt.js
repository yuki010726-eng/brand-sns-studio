...
export function buildPrompt(card, conceptId, opts = {}) {
  ...
  return [
    ...
    localeFor(card.shot),
    isBlog
      ? `landscape Korean corporate PR campaign composition: ${blogLayout}. Let the subject, brand character, and lighting determine the palette; do not default to navy-and-white, blue-and-white, or any preset corporate color scheme.`
    ...
    isBlog
      ? `Create the finished Korean PR banner as one flattened image, matching the supplied editorial references: all typography must be embedded in the image, not added later. Render these Korean strings exactly, with correct Hangul spelling and line breaks chosen for a polished layout: ${blogCopy}. Choose typography color, background treatment, and information-panel treatment for legibility and the subject's own visual character; do not impose a navy-and-white palette or a fixed left-copy/right-visual layout. Do not render Instagram handles, @mentions, social-media account names, footer handles, placeholder text, gibberish, watermarks, or logos. Do not add any other words.`
      : '',
    card.shot,
    opts.subject
      ? `THE SUBJECT IS WHAT MATTERS MOST: the photo must show what this Korean sentence is about: "${opts.subject}". Show the concrete object or action it refers to. If the style above pulls toward a generic mood, the subject wins.${isBlog ? ' The supplied banner copy is the only text allowed in the image.' : ' Never render the sentence as text in the image'}`
      : '',
  ].filter(Boolean).join('. ');
}
