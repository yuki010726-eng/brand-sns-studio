...
export async function getInstagramProfilePreview(instagramUserId) {
  const body = await authorizedRequest(`/api/instagram/profile?instagram_user_id=${encodeURIComponent(instagramUserId)}`);
  return { profile: body.profile || {}, media: body.media || [] };
}
