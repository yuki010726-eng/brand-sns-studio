...
// The text and image tabs mount independently. Keep their first result shared
// until the user explicitly requests another set.
const topicPresetCache = new Map();

/** DB의 최신 추천 주제 중 무작위 4개를 반환한다. */
export async function loadRandomTopicPresets(productId, { force = false } = {}) {
  if (!productId) return [];
  if (!force && topicPresetCache.has(productId)) {
    return topicPresetCache.get(productId);
  }
  ...
  const topics = Array.isArray(data?.prompt_settings?.topicPresets)
    ? data.prompt_settings.topicPresets
        ...
        .filter(Boolean)
    : [];
  const presets = shuffled([...new Set(topics)]).slice(0, 4);
  topicPresetCache.set(productId, presets);
  return presets;
}
