...
export async function loadProducts() {
  ...
  const products = (data || []).map((row) => {
    ...
    // 쓰레드 말투 예문을 블로그·카드의 승인 마무리로 재사용하지 않는다.
    // 「연말 정리하실 때 참고만 하세요」 같은 채널 전용 문장이 카드 제목 재료로
    // 올라오던 원인이었다. 공용 마무리가 필요하면 settings.closings에 명시한다.
    const closings = list(settings.closings);
