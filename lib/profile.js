 ...
 */
const AITVCF = {
  subtitle: ['AI 광고제작', 'TV광고 지원', 'IPTV 광고', '중소기업 광고'],
  slogan: [
    'TV 광고, 이제 중소기업도 합니다',
    '제작비 걱정 없이 TV로',
    '만들고 내보내는 것까지 한 번에',
...
export const AITVCF_BRAND = (() => {
  ...
  return p ? { id: p.id, label: p.name, en: BRAND_EN.aitvcf, short: p.short, slug: AWARD_SLUG.aitvcf } : null;
})();
...
export function buildProfile({ typeId, brandId, seed = 0, withLogo = false }) {
  ...
  if (type === 'aitvcf' && AITVCF_BRAND) {
    ...
    return {
      ...
      link: littlyUrl(AITVCF_BRAND.slug), imagePrompt: awardPrompt(s, logo), withLogo: logo,
    };
  }
...
function awardsDraft(s, brand, isAd = false) {
  ...
  const subtitle = pick(set.subtitle, s);
   ...
   */
  const name = s % 2 === 0 ? brand.en : `${brand.en} | ${subtitle}`;
