 ...
 *
 * ⚠️ 어워즈형은 Supabase의 승인된 상품에서 이름과 사실을 가져온다.
 ...
 *
 * ⚠️ **마케터형은 다시 하나로 합쳤다** (요청자 지시 2026-08-14).
 *    2026-08-14 초반에 잠깐 '마케터 (심볼)'·'마케터 (사진)' 로 나눴는데, 유형 칸이 4개가 되면서
 *    한 줄에 못 들어가 두 줄로 접혔다. 아바타를 마크로 갈지 얼굴로 갈지는 브랜드 어워즈의
 *    브랜드 선택과 같은 성격의 결정이라, **어워즈 칸 안의 브랜드 칩과 같은 자리**(칸 안의
 *    하위 선택)로 옮겼다 — 아래 `MARKETER_STYLES` 참고.
 */
export const PROFILE_TYPES = [
  ...
  },
  {
    id: 'marketer',
    label: '마케터',
    desc: '정보·노하우 계정. 아바타를 심볼로 할지 인물로 할지 아래에서 고르세요',
  },
];

/**
 * 마케터형 아바타 방식 — 어워즈의 `AWARD_BRANDS` 와 같은 자리(칸 안의 하위 선택)에 쓴다.
 * 이름·소개는 두 방식이 같고 **이미지 프롬프트만 갈린다** (`marketerDraft()` 를 함께 쓴다).
 */
export const MARKETER_STYLES = [
  {
    id: 'symbol',
    label: '심볼',
...
/** 사진을 첨부해야 프롬프트가 제 일을 하는 방식인지 — 화면이 업로드 칸을 띄우는 기준이다 */
export const needsPhoto = (profile) =>
  profile?.typeId === 'marketer' &&
  MARKETER_STYLES.find((m) => m.id === profile?.marketerStyle)?.needsPhoto === true;
...
export const AWARD_BRANDS = [];

/** AI TV CF 유형 — 고를 것이 없으므로 상품 하나로 고정한다 */
export let AITVCF_BRAND = null;

export function refreshProfileProducts() {
  const awards = PRODUCTS.filter((p) => p.id !== 'aitvcf').map((p) => ({
    id: p.id,
    label: p.name,
  ...
  AWARD_BRANDS.splice(0, AWARD_BRANDS.length, ...awards);

  const p = PRODUCTS.find((product) => product.id === 'aitvcf');
  AITVCF_BRAND = p
    ? { id: p.id, label: p.name, en: BRAND_EN.aitvcf, enShort: BRAND_EN_SHORT.aitvcf, short: p.short, slug: AWARD_SLUG.aitvcf }
    : null;
}
 ...
 * 유형에 맞는 프로필 초안을 만든다.
 *
 * @param {{typeId:'awards'|'aitvcf'|'marketer', brandId?:string, seed?:number,
 *          withLogo?:boolean, marketerStyle?:'symbol'|'photo'}} opts
 *        withLogo — 어워즈형 전용. 로고를 첨부해 쓸 프롬프트로 만든다. 마케터형은 무시된다.
 *        marketerStyle — 마케터형 전용. 심볼로 갈지 인물 사진으로 갈지. 그 외 유형은 무시된다.
 * @returns {{typeId:string, brandId:string|null, marketerStyle:string|null, name:string, bio:string,
 *            slug:string, link:string, imagePrompt:string, withLogo:boolean}}
 */
export function buildProfile({ typeId, brandId, seed = 0, withLogo = false, marketerStyle = 'symbol' }) {
  ...
  if (type === 'aitvcf' && AITVCF_BRAND) {
    ...
    return {
      typeId: type, brandId: AITVCF_BRAND.id, marketerStyle: null,
  ...
  if (type === 'awards') {
    ...
    const logo = Boolean(withLogo);
    return {
      typeId: type, brandId: brand.id, marketerStyle: null,
  ...
  // 마케터 두 방식은 이름·소개가 같고 이미지 프롬프트만 갈린다
  const style = MARKETER_STYLES.find((m) => m.id === marketerStyle) ? marketerStyle : 'symbol';
  const slug = pick(MARKETER.slug, s);
  const { name, bio } = marketerDraft(s, slug);
  return {
    typeId: type, brandId: null, marketerStyle: style,
    name: clampName(name), bio, slug,
    link: littlyUrl(slug),
    imagePrompt: style === 'photo' ? marketerPhotoPrompt(s) : marketerPrompt(s),
    withLogo: false,
  };
}
