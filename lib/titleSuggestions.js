...
export function titleSuggestionKey(product, state) {
  ...
  return cacheKey("title-v2", JSON.stringify({
...
export function cachedTitleSuggestions(product, state) {
  const key = titleSuggestionKey(product, state);
  return titleSuggestionCache.get(key) || readSuggestion(key) || null;
}
...
export function cacheTitleSuggestions(product, state, titles) {
  ...
  if (cleaned.length) {
    const key = titleSuggestionKey(product, state);
    titleSuggestionCache.set(key, cleaned);
    writeSuggestion(key, cleaned);
  }
  return cleaned;
}
...
export async function getTitleSuggestions(product, state, options = {}) {
  ...
  if (options.forceRefresh) {
    titleSuggestionCache.delete(key);
    clearSuggestion(key);
  }
...
${proofs.length ? proofs.map((item) => `- ${item}`).join("\n") : "- 별도 근거 없음. 주어진 상품명과 주제 범위만 사용"}`;
  ...
  const raw = await generateText(prompt, {
    ...
    maxOutputTokens: 260,
    temperature: 0.9,
    usageType: "title_recommendation",
