 ...
 *
 * ⚠️ **블로그마다 완전히 다른 네이버 계정일 수 있다** (2026-09-15, `naver_accounts`
 *    테이블 도입). 그래서 세션도 계정 수만큼 따로 있어야 한다 — `openBrowser(profileId)`
 *    의 `profileId` 를 주면 `.naver-profile/{profileId}/` 를, 안 주면 예전처럼
 *    공용 `.naver-profile/` 를 연다. `profileId` 는 `naver_accounts.id`(uuid) 그대로다 —
 *    사용자가 입력하는 blogId·별명을 폴더 이름에 쓰면 경로 조작 위험이 있어 쓰지 않는다.
 *    계정마다 `npm run naver:setup -- {id}` 로 그 폴더에 한 번씩 직접 로그인해야 한다.
 */
import { chromium } from "playwright";
import path from "node:path";
import { parseBlogDoc } from "./blogDoc.js";

const PROFILE_ROOT = path.resolve(process.cwd(), ".naver-profile");
// 폴더 이름으로 쓰이므로 방어적으로 한 번 더 제한한다 — 실제로는 항상 Postgres가
// 만든 uuid(naver_accounts.id)만 여기로 들어온다(app/api/naver/fill/route.jsx 참고).
const SAFE_PROFILE_ID_RE = /^[A-Za-z0-9_-]{1,64}$/;

function profileDirFor(profileId) {
  if (!profileId) return PROFILE_ROOT;
  if (!SAFE_PROFILE_ID_RE.test(profileId)) {
    throw new Error("프로필 id 형식이 올바르지 않습니다.");
  }
  return path.join(PROFILE_ROOT, profileId);
}
 ...
 * `profileId` 를 주면 그 계정 전용 폴더(`.naver-profile/{profileId}/`)를 연다 — 안 주면
 * 예전부터 쓰던 공용 폴더(`.naver-profile/`)를 연다(레거시 단일 계정 경로, 그대로 유지).
 */
export async function openBrowser(profileId) {
  const context = await chromium.launchPersistentContext(profileDirFor(profileId), {
    headless: false,
 ...
 *
 * ⚠️ **실패해도 던지지 않는다** (2026-09-15, 실제로 겪은 버그 — 원고에 소제목(■)이
 *    있으면 채우다가 중간에 멈추고 브라우저가 닫힌다는 신고). 예전엔 여기서 던진
 *    예외가 `fillBody()` 를 통째로 끊고 `route.jsx` 의 catch 까지 올라가 **그때까지
 *    채운 내용째로 브라우저를 닫아버렸다.** 이미지 삽입은 실패해도 자리표시자로
 *    대신하고 계속 진행하는데(`insertImage` 호출부의 try/catch), 소제목 서식 전환만
 *    무방비였다. 지금은 툴바를 못 찾아도 **서식 없이 평문으로라도 계속 채운다** —
 *    글자가 남는 것이 브라우저가 통째로 닫혀 아무것도 안 남는 것보다 낫다.
 */
async function setParagraphFormat(page, kind) {
  try {
    // ⚠️ 8000ms였다가 15000ms로 올렸다(2026-09-21). 선택자 자체는 실제 로그인 세션에서
    //    여전히 정확히 일치하는데(직접 재확인함), 에디터가 막 뜬 직후(cold)에는 이 버튼이
    //    나타나기까지 8초를 넘기는 경우가 실제로 있었다 — 소제목이 평문으로 채워지는
    //    증상이 여기서 났다. 버튼이 늦게 뜨는 것뿐이라 시간만 더 주면 된다.
    const dropdownBtn = await page.waitForSelector(".se-text-format-toolbar-button", { timeout: 15000 });
    ...
    console.error(`[naverPublish] 문단 서식(${kind}) 전환 실패 — 서식 없이 계속합니다:`, err.message);
  }
}
...
export async function insertImage(page, frame, dataUrl) {
  ...
  // ⚠️ 15000ms → 20000ms(2026-09-21). 버튼·파일선택창 자체는 실제 세션에서 문제없이
  //    뜬다 — 에디터가 막 열린 직후에는 가끔 늦게 반응해서(위 setParagraphFormat과 같은
  //    원인) 여유를 더 준다.
  const fileChooserPromise = page.waitForEvent("filechooser", { timeout: 20000 });
  const imgBtn = await page.waitForSelector(".se-image-toolbar-button", { timeout: 20000 });
  await imgBtn.click();
 ...
 *
 * ⚠️ **블록 하나가 실패해도 전체를 멈추지 않는다** (2026-09-15). 예전엔 블록 처리 중
 *    아무 예외나 나면(주로 `setParagraphFormat`, 그 외 드물게 타이핑·클릭 자체) 그
 *    예외가 `fillBody()` 밖으로 그대로 튀어 `route.jsx` 의 catch 로 올라갔고, 거기서
 *    **그때까지 채운 내용째로 브라우저를 닫아버렸다** — "일부만 채우다 멈추고
 *    브라우저가 닫힌다"는 신고가 실제로 이 경로였다. 이제 블록마다 감싸서, 실패한
 *    블록은 건너뛰고 나머지를 계속 채운다. 완벽하지 않아도 사람이 검토하고 고쳐서
 *    발행할 수 있는 글이 남는 것이 브라우저가 통째로 닫혀 아무것도 안 남는 것보다 낫다
 *    (8-절의 "형식은 반려하지 않고 고친다"와 같은 방향의 판단).
 */
export async function fillBody(page, frame, rawDraft, images = {}, imageLayout = {}) {
  ...
  for (const [blockIndex, block] of doc.blocks.entries()) {
    try {
      await fillOneBlock(page, frame, block, images, imageLayout);
    } catch (err) {
      console.error(
        `[naverPublish] 블록 ${blockIndex}번(${block.type}) 처리 실패 — 건너뛰고 계속합니다:`,
        err.message,
      );
    }
  }
...
async function fillOneBlock(page, frame, block, images, imageLayout) {
  if (block.type === "image") {
