 ...
 */
export const outlineKeyOf = (s) => `${s.productId}|${String(s.topic || '').trim()}|${String(s.focusPoint || '').trim()}|${s.tone}`;
...
export async function generateOutline(ctx, opts = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (parsed?.points) {
      ...
      parsed.points = parsed.points.map((x) => ({ q: trimWords(x?.q, Q_MAX), a: trim(x?.a, A_MAX), shot: trim(x?.shot, SHOT_MAX), icon: trim(x?.icon, SHOT_MAX) }));
    }
