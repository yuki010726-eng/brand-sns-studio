...
function factSheet(p) {
  ...
  const proofs = (p.productProofs || p.voice?.proof || []).map((proof) => {
  ...
  return [
    ...
    proofs.length ? `product_proofs의 검증된 사실:\n${proofs.join('\n')}` : '',
...
function buildPrompt(ctx, retryNote) {
  ...
  return [
    ...
    p.closings.length
      ...
      : '- closing 은 이 글에서 확인한 핵심 사실로 끝냅니다. 막연한 권유나 인사말을 쓰지 않습니다.',
    '',
    '── 상품 자료 ──',
    factSheet(p),
    '',
    ...(p.closings.length
      ? ['허용된 마무리 문장 (이 중 하나를 그대로):', ...p.closings.map((s) => `- ${s}`)]
      : []),
    '── 자료 끝 ──',
    '',
    ...
    p.closings.length
      ...
      : `  "closing": "이 글에서 확인한 핵심 사실로 끝나는 한 문장",`,
...
function validate(o, ctx) {
   ...
   */
  if ((p.closings || []).length && !approvedClosing(o.closing, p)) {
...
export function mergeCore(ruleCore, outline) {
  ...
  const approvedList = ruleCore.product.closings || [];
  const approved = approvedList.find((c) => flatten(c) === flatten(outline.closing));
  const closing = approvedList.length ? (approved || ruleCore.closing) : outline.closing;
  ...
  return {
    ...
    points: outline.points.map((x, i) => {
      ...
      return { ...base, q: x.q, a: x.a, shot: x.shot || base.shot, icon: x.icon || base.icon };
    }),
