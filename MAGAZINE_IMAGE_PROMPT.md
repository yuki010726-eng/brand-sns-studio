...
## 10. 2026-08-27 실측 — 「시간·기한」 주제에서 걸린 것

주제 「접수 마감까지 2주 남았습니다」로 A·B·C를 여러 판 뽑으며 확인한 것들이다.
전부 **구도는 맞는데 사진이 딴말을 한** 경우라, 4절 표만으로는 안 걸러진다.

### ① 밀도를 채우는 방법이 구도를 바꾼다

카드형 3절의 「작은 요소를 반복시킨다」는 **「범위·규모」 구도의 장치**다.
그걸 「시간·기한」 주제에 썼더니 서류 트레이가 줄줄이 반복되면서
**「접수가 많다」는 사진**이 됐다. 시계는 벽 저쪽에 걸려 아무 관계도 못 맺었다.

**반복은 「범위·규모」에서만 쓴다.** 계측기·선택·비교 구도에서 화면을 채워야 하면
반복이 아니라 **그 장소가 돌아가는 데 필요한 집기**로 채운다 — 접수함 · 양식 더미 ·
펜 · 도장. 개수가 아니라 종류로 채우는 것이다.

### ② 계측기 구도는 두 사물을 한 동작으로 묶어야 한다

4절 표의 「시간·기한 = 계측기 + 대상 하나」는 **두 사물을 나란히 놓으라는 말이 아니다.**
시계를 벽에 걸고 봉투를 카운터에 두면 「시간」과 「대상」이 각각 따로 있는 두 사물이 되고,
사진은 그냥 정물이 된다.

**계측기가 대상에 닿아 있고, 대상이 무언가 하는 중이어야 한다.**
통과한 형태 — 봉투가 투입구로 반쯤 들어간 상태에서 시계가 그 상자에 기대어 닿아 있다.

### ③ 기하가 모호하면 행위가 사라진다

「투입구를 카운터 **앞면**에 낸다」 + 「카메라는 **카운터 높이**에서」로 썼더니
앞면(수직)인지 상판(수평)인지 모델이 판단하지 못하고, 상판 위에 황동 레일이 얹힌
정체불명의 물건이 나왔다. **봉투가 어디로 들어가는지 안 읽히면 「접수」라는 행위 자체가 없어진다.**

통과한 형태는 이것이다. 새 주제에서도 넣고 빼는 장면이면 이 형태를 먼저 쓴다.

```
구멍은 상자의 평평한 뚜껑에 낸다 (방향이 하나뿐이라 헷갈릴 수 없다)
대상은 반쯤 들어간 상태로 둔다 (아래로 간다는 것이 형태로 읽힌다)
카메라는 카운터보다 조금 위에서 3/4 각도
"뚜껑 · 구멍 · 앞면이 한 화면에 동시에 보여야 한다" 를 문장으로 못박는다
```

⚠️ 사물의 **면**(앞면·윗면·옆면)을 지정할 때는 **카메라 각도와 같이 검산한다.**
   그 각도에서 그 면이 실제로 보이는지 확인하지 않으면 모델이 아무 데나 붙인다.

### ④ 매거진형과 카드형은 기하를 공유하고 재질·조명만 바꾼다

문서가 원래 말하던 「구도는 같고 재질·조명만 다르다」가 **실측으로 확인됐다.**
카드형에서 통과한 기하(뚜껑 구멍 + 반쯤 들어간 봉투)를 매거진형에 그대로 옮기고
재질만 dark stone · matte black · brushed steel 로, 조명만 하드라이트로 바꿨더니 통과했다.

**한쪽이 통과하면 다른 쪽은 기하를 베끼는 것이 정답이다.** 새로 짜지 말 것.
⚠️ 단 카드형은 3절(밀도)이 더 붙는다 — 밝은 톤에서는 같은 기하라도 화면이 더 차야 한다.
